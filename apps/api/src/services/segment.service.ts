...
import { Prisma, Segment, SegmentRule, SegmentRuleType, SegmentEntityType } from '@prisma/client';
...
export class SegmentService {
  ...
  private buildContactWhereClause(segment: Segment & { rules: SegmentRule[] }): Prisma.ContactWhereInput {
  ...
  private resolveRuleToWhere(rule: SegmentRule): Prisma.ContactWhereInput | null {
  ...
  private buildKeywordClause(rule: SegmentRule): Prisma.ContactWhereInput | null {
  ...
  private buildStringClause(field: 'city' | 'state' | 'pincode', rule: SegmentRule): Prisma.ContactWhereInput | null {
  ...
  private parseStringArray(value: Prisma.JsonValue | null): string[] {
    if (Array.isArray(value)) {
      return value
        .map((entry) => (typeof entry === 'string' ? entry.trim() : typeof entry === 'number' ? String(entry) : null))
