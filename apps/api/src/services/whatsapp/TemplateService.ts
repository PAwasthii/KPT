...
export class WhatsappTemplateService {
  ...
  async listTemplates(accountId: number) {
    ...
    return templates.map((tpl) => {
      ...
      return {
        ...
        languages: componentsData?.languages || (tpl.language ? [{
          code: tpl.language,
          status: tpl.status,
          id: tpl.id
        }] : []),
  ...
  private async processTemplates(templates: any, account: any) {
    const templateRecords: Msg91Template[] = Array.isArray(templates) ? templates : [];

    // Extract template names from MSG91 response
    const msg91TemplateNames = templateRecords.map((tpl) =>
    ...
    console.log('Templates to archive:', templatesToArchive.map((t) => ({ id: t.id, name: t.name })));
    ...
    const templatesToUnarchive = localTemplates.filter(
      (local) => msg91TemplateNames.includes(local.name)
    );
    ...
    if (templatesToUnarchive.length > 0) {
      ...
      console.log(`Unarchived ${templatesToUnarchive.length} templates that are back in MSG91`);
    }
    ...
    for (const tpl of templateRecords) {
      ...
      const templateName = tpl.template_name || tpl.name || tpl.templateId || 'unknown';

      // Get primary language (first one if multiple, or single language)
      const primaryLanguage = Array.isArray(tpl.languages) && tpl.languages.length > 0 && tpl.languages[0]
      ...
      console.log(`Upserting template: ${templateName} with languages: ${
      ...
      const componentsData: Record<string, any> = {
        ...
        languages: tpl.languages || (primaryLanguage ? [{
  ...
  async createTemplate(accountId: number, templateData: {
    template_name: string;
    language: string;
    category: string;
    button_url?: boolean;
   ...
   */
  async uploadMedia(accountId: number, mediaBuffer: Buffer, mimeType: string): Promise<string> {
    const account = await this.accountService.getAccountOrThrow(accountId);
    const integratedNumber = account.senderId || account.phoneNumber;
    if (!integratedNumber) {
      throw new Error('Account is missing sender id/phone number');
  ...
  async updateTemplate(accountId: number, templateName: string, components: any[]) {
    const account = await this.accountService.getAccountOrThrow(accountId);
    const integratedNumber = account.senderId || account.phoneNumber;
    if (!integratedNumber) {
      throw new Error('Account is missing sender id/phone number');
