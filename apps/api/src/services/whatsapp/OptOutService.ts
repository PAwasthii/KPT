...
export class OptOutService {
   ...
   */
  private async syncOptOutToEntities(phone: string, channel: Channel, optedOut: boolean) {
  ...
  async listOptOuts(params: {
    ...
    const { channel, search, skip = 0, take = 50, sortBy = 'optedOutAt', sortOrder = 'desc' } = params;
   ...
   */
  async bulkImportOptOuts(phones: string[], channel: Channel, source: string = 'bulk_import') {
    const normalized = phones.map((p) => this.normalizePhone(p));
