...
export class Msg91Client {
  ...
  constructor(options: Msg91ClientOptions) {
    ...
    this.baseUrl = options.baseUrl || process.env.MSG91_BASE_URL || 'https://api.msg91.com/api/v5';
  }

  async fetchTemplates(sender: string) {
    const url = new URL(`${this.baseUrl}/whatsapp/get-template-client/${sender}`);
    ...
    console.log('MSG91 fetchTemplates raw response:', JSON.stringify(response, null, 2));

    // MSG91 returns nested structure: { data: [{ name, category, languages: [...] }] }
    // Group by template name and collect all languages
    const templatesData = Array.isArray(response?.data) ? response.data : response?.templates ?? [];
    ...
    console.log(`Grouped ${groupedTemplates.length} templates from ${templatesData.length} base templates`);
  ...
  async createTemplate(payload: Msg91CreateTemplatePayload) {
    ...
    const bodyComponent = Array.isArray(apiPayload.components)
      ? apiPayload.components.find((component: any) => component?.type === 'BODY')
      : null;
    if (bodyComponent) {
      console.log('BODY component being sent to MSG91:', JSON.stringify(bodyComponent, null, 2));
    } else {
   ...
   */
  async uploadSampleMedia(integratedNumber: string, mediaBuffer: Buffer, mimeType: string): Promise<string> {
    ...
    formData.append('media', blob, `media.${this.getExtensionFromMimeType(mimeType)}`);
    ...
    if (!response.ok) {
      ...
      throw new Error(`MSG91 media upload failed (${response.status}): ${text}`);
    }
    ...
    if (result.status !== 'success' || !result.data?.url) {
      throw new Error('Failed to upload media: ' + (result.errors || 'Unknown error'));
    }
  ...
  private buildComponentsArray(variables?: Record<string, unknown>): any[] {
    ...
    console.log('🔧 buildComponentsArray INPUT:', JSON.stringify(variables, null, 2));
      ...
      if (value && typeof value === 'object') {
        ...
        if (key.startsWith('header_')) {
          if (objValue.type === 'image' && objValue.image) {
            ...
            const isUrl = typeof mediaValue === 'string' && (mediaValue.startsWith('http://') || mediaValue.startsWith('https://'));
    ...
    console.log('🔧 buildComponentsArray OUTPUT:', JSON.stringify(components, null, 2));
    return components;
  }
  ...
  private buildComponents(variables?: Record<string, unknown>) {
    ...
    console.log('🔧 buildComponents INPUT:', JSON.stringify(variables, null, 2));

    const result = Object.entries(variables).reduce<Record<string, Record<string, unknown>>>(
      (acc, [key, value]) => {
        if (value && typeof value === 'object') {
          const objValue = value as Record<string, unknown>;
