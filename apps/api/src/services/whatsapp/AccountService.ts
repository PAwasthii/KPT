...
export class WhatsappAccountService {
  ...
  async getAccountOrThrow(id: number): Promise<WhatsAppNumber & { apiKey: string }> {
    const record = await prisma.whatsAppNumber.findUnique({ where: { id } });
    if (!record) {
      throw new Error('WhatsApp account not found');
    }

    const apiKey = decryptSecret(record.encryptedApiKey, record.iv, record.authTag);
    ...
    for (const number of numbers) {
      try {
        ...
        const phoneNumber = this.normalizePhone(
          number.phone_number || number.phoneNumber || number.integrated_number || ''
        );
