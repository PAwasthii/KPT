...
export class WhatsappWebhookService {
  ...
  async handleMsg91InboundMessage(payload: Msg91WebhookPayload) {
    ...
    const phoneNumber = this.getStringValue(payload, 'customerNumber')
    ...
    const messageText = this.getStringValue(payload, 'text')
    ...
    if (!isStopMessage) {
      ...
      return { success: true, message: 'Message is not a STOP command', isOptOut: false };
    }
    ...
    const userName = contact?.name || (lead ? `${lead.firstName} ${lead.lastName || ''}`.trim() : null);
    ...
    try {
      ...
      if (contact || lead) {
        ...
        if (recentDelivery) {
          ...
          console.log(`📧 Found recent campaign: ${recentDelivery.campaign?.name} (ID: ${campaignId})`);
        } else {
          ...
          if (!systemCampaign) {
            ...
            systemCampaign = await prisma.campaign.create({
              data: {
                name: 'System Events',
                description: 'System-generated events (opt-outs, unsubscribes, etc.)',
        ...
        console.log(`✅ Created analytics event for ${contact ? 'contact' : 'lead'} ID: ${contact?.id || lead?.id}`);
        ...
        console.log('⚠️  No contact or lead found for phone number. Opt-out recorded but no activity timeline entry created.');
      }
  ...
  async handleMsg91Event(payload: Msg91WebhookPayload) {
    ...
    const correlationId = this.getStringValue(payload, 'requestId')
    ...
    const customerNumber = this.getStringValue(payload, 'customerNumber')
      || this.getStringValue(payload, 'customer_number')
    ...
    if (deliveries.length > 0) {
      console.log('First delivery contactId:', deliveries[0]?.contactId, 'leadId:', deliveries[0]?.leadId);
    }

    if (!deliveries.length) {
      console.log('No matching deliveries found for provider IDs:', providerIds, 'and phone:', customerNumber);
      return { storedId: stored.id, matched: 0 };
    }
    ...
    const errorMessage = this.getStringValue(payload, 'reason')
    ...
    await Promise.all(
      deliveries.map((delivery) => {
        ...
        console.log(`Delivery ${delivery.id}: current=${delivery.status}(${currentRank}), new=${statusUpdate.status}(${newRank}), shouldUpdate=${shouldUpdateStatus}`);
        ...
        return prisma.campaignDelivery.update({
          ...
          data: {
            ...
            deliveredAt:
              statusUpdate.status === CampaignDeliveryStatus.DELIVERED ? new Date() : delivery.deliveredAt,
            readAt:
              statusUpdate.status === CampaignDeliveryStatus.READ ? new Date() : delivery.readAt,
            failedAt:
              statusUpdate.status === CampaignDeliveryStatus.FAILED ? new Date() : delivery.failedAt,
            errorMessage: statusUpdate.status === CampaignDeliveryStatus.FAILED ? errorMessage : delivery.errorMessage,
        ...
        deliveries.map(async (delivery) => {
          ...
          if (phone) {
            try {
              ...
              console.log(`✓ Added opt-out for phone: ${phone} from campaign: ${delivery.campaignId}`);
            } catch (error) {
              console.error(`✗ Failed to add opt-out for phone: ${phone}`, error);
            }
          }
        })
      );
    ...
    const contactIds = deliveries.map(d => d.contactId).filter((id): id is number => id !== null);
    const leadsFromContacts = contactIds.length > 0
    ...
    await Promise.all(
      deliveries.map((delivery) => {
        ...
        const leadId = delivery.leadId || (delivery.contactId ? contactToLeadMap.get(delivery.contactId) : null) || null;
        ...
        return prisma.analyticsEvent.create({
          data: {
            ...
            eventData: {
              campaignName: campaignNameMap.get(delivery.campaignId) || 'Unknown Campaign',
  ...
  private getStringValue(source: Record<string, unknown> | undefined, key: string): string | null {
