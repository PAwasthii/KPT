...
export class WhatsappSendService {
  ...
  async createCampaign(input: CreateCampaignInput) {
    const account = await this.accountService.getAccountOrThrow(input.accountId);
    const template = await this.templateService.findTemplateByName(account.id, input.templateName);
    if (!template) {
    ...
    if (!isDraft) {
      ...
      if (input.audience === 'upload' && csvContactsData.length > 0) {
        ...
        const { allowed: allowedPhones, blockedCount } = await optOutService.filterOptedOut(
          allPhones,
          'whatsapp'
        );

        if (blockedCount > 0) {
          console.log(`⚠️  Filtered out ${blockedCount} opted-out numbers from CSV upload`);
        }
      ...
      if (!deliveriesData.length) {
        throw new Error('None of the selected contacts have a valid phone number');
      }
  ...
  async updateCampaign(campaignId: number, input: UpdateCampaignInput) {
    ...
    if (input.messageParams) {
      configUpdates.messageParams = input.messageParams as Prisma.InputJsonValue;
    }
  ...
  async sendCampaign(campaignId: number) {
    ...
    const account = await this.accountService.getAccountOrThrow(config.whatsappNumberId);
    ...
    if (!integratedNumber) {
      throw new Error('WhatsApp account is missing the integrated phone number');
    }

    const rawDeliveries = await prisma.campaignDelivery.findMany({
      where: {
        campaignId,
        status: { in: [CampaignDeliveryStatus.PENDING, CampaignDeliveryStatus.FAILED] },
      },
      include: {
        contact: true,
      },
    ...
    let deliveries: DeliveryWithContact[] = rawDeliveries.map(delivery => ({
      ...delivery,
      csvData: delivery.csvData ? (delivery.csvData as Record<string, string>) : null,
    }));
    ...
    if (blockedCount > 0) {
      console.log(`⚠️  Found ${blockedCount} opted-out numbers at send time. Marking as OPTED_OUT.`);

      // Get blocked phone numbers
      const allowedPhoneSet = new Set(allowedPhones);
      const blockedDeliveries = deliveries.filter(d => !allowedPhoneSet.has(d.address));
    ...
    if (!deliveries.length) {
      return { queued: 0, message: 'All deliveries were opted out', blockedCount };
    }
    ...
    const templateLanguage = config.language || config.template.language || 'en';

    // Extract namespace from template components data
    // Try multiple possible locations for namespace
    const templateComponents = config.template.components as Record<string, unknown> | null;
    const templateComponentsJson = (config.template as Record<string, unknown>)?.componentsJson as
    ...
    if (!templateNamespace && templateComponentsJson) {
      templateNamespace = templateComponentsJson?.namespace as string | undefined;
    }
    ...
    if (!templateNamespace) {
      console.warn('⚠️  WARNING: Template namespace is missing. This might cause MSG91 API to reject the request.');
      console.warn('Template components:', JSON.stringify(config.template.components, null, 2));
      console.warn('Template componentsJson:', JSON.stringify(templateComponentsJson, null, 2));
    }
    ...
    console.log('Message params:', JSON.stringify(config.messageParams, null, 2));
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      try {
        const recipients = batch.map((delivery) => {
          ...
          const csvData = delivery.csvData ? (delivery.csvData as Record<string, string>) : undefined;
          ...
          this.applyTemplateSpecificVariables(variables, config.template.category);
          console.log(`Recipient ${delivery.address}:`, JSON.stringify(variables, null, 2));
          return {
        ...
        console.log('Sending to MSG91 with payload:', JSON.stringify({
          templateName: config.template.name,
          templateLanguage,
          templateNamespace,
        ...
        queued += batch.length;
        console.log(`✅ Batch ${i + 1} sent successfully. Queued: ${batch.length}`);
        ...
        const errorMessage = error instanceof Error ? error.message : 'MSG91 send failed';
    ...
    const enriched = await Promise.all(
      campaigns.map(async (campaign) => {
        ...
        return {
          ...
          deliveryStats: stats,
          templateName: config?.template?.name,
          scheduledAt: config?.scheduledAt ?? campaign.whatsappConfig?.scheduledAt ?? null,
        };
      })
    );
  ...
  private async resolveContacts(audience: AudienceType, segmentId?: number): Promise<ContactRecord[]> {
    if (audience === 'segment') {
      if (!segmentId) {
    ...
    Object.entries(variables || {}).forEach(([key, value]) => {
      ...
      if (value && typeof value === 'object') {
        ...
        if (typeof typedValue.value === 'string') {
          typedValue.value = this.interpolate(typedValue.value, contact, csvData);
        }
  ...
  private interpolate(template: string, contact: ContactRecord | null, csvData?: Record<string, string>) {
    let result = template;
    ...
    if (contact) {
      ...
      result = result
        ...
        .replace(/\{\{\s*(?:contact\.)?countryCode\s*\}\}/gi, contact?.countryCode || '')
        .replace(/\{\{\s*(?:contact\.)?city\s*\}\}/gi, contact?.city || '')
        .replace(/\{\{\s*(?:contact\.)?state\s*\}\}/gi, contact?.state || '')
        .replace(/\{\{\s*(?:contact\.)?pincode\s*\}\}/gi, contact?.pincode || '')
        .replace(/\{\{\s*(?:contact\.)?position\s*\}\}/gi, contact?.position || '')
  ...
  private applyTemplateSpecificVariables(
    ...
    if (!templateCategory || templateCategory.toUpperCase() !== 'AUTHENTICATION') {
      return;
    }
    ...
    const otpValue = extractValue(variables['button_1']) || extractValue(variables['body_1']);
    if (!otpValue) {
      return;
    }
