...
export class ApprovalController {
  ...
  /**
   * GET /api/approvals
   * Get all approvals in the system (ADMIN, SYSTEM_ADMIN only)
    ...
    try {
      ...
      if (status) {
        ...
        if (!valid.includes(status as ApprovalStatus)) {
          return handleValidationError(res, `Invalid status. Must be one of: ${valid.join(', ')}`, 'status', operation);
        }
        where.status = status;
      }

      if (targetObjectName) {
        const valid: ApprovalTargetObject[] = ['OPP', 'QUOTE'];
        if (!valid.includes(targetObjectName as ApprovalTargetObject)) {
          return handleValidationError(res, `Invalid targetObjectName. Must be one of: ${valid.join(', ')}`, 'targetObjectName', operation);
        }
  ...
  async getApprovalById(req: Request, res: Response) {
    ...
    try {
      ...
      if (isNaN(id)) return handleValidationError(res, 'Invalid approval ID', 'id', operation);
  ...
  async getMyApprovals(req: Request, res: Response) {
    ...
    try {
      ...
      if (!validTypes.includes(type)) {
        return handleValidationError(res, `Invalid type. Must be one of: ${validTypes.join(', ')}`, 'type', operation);
      }
  ...
  /**
   * POST /api/approvals
   * Manually raise an approval request (ADMIN, SYSTEM_ADMIN only)
  ...
  async createApproval(req: Request, res: Response) {
    ...
    try {
      const userId = req.user!.id;
      const { targetObjectName, targetRecordId, requestedToId, comment } = req.body;

      // Validate required fields
      if (!targetObjectName || !targetRecordId || !requestedToId) {
        return handleValidationError(res, 'targetObjectName, targetRecordId, and requestedToId are required', 'body', operation);
      }

      const validTargets: ApprovalTargetObject[] = ['OPP', 'QUOTE'];
      if (!validTargets.includes(targetObjectName)) {
        return handleValidationError(res, `Invalid targetObjectName. Must be one of: ${validTargets.join(', ')}`, 'targetObjectName', operation);
      }

      const parsedRecordId = parseInt(targetRecordId, 10);
      if (isNaN(parsedRecordId)) {
        return handleValidationError(res, 'Invalid targetRecordId', 'targetRecordId', operation);
      }
      ...
      if (targetObjectName === 'OPP') {
        const opportunity = await prisma.opportunity.findUnique({
          ...
          select: { id: true, name: true, opportunityNumber: true, deletedAt: true },
        });
        ...
        if (quote.status !== 'DRAFT') {
          return handleValidationError(res, `Quote is not in an approvable state. Current status: ${quote.status}`, 'targetRecordId', operation);
        }
      ...
      const existingApproval = await prisma.approvalProcess.findFirst({
        where: { targetObjectName, targetRecordId: parsedRecordId, status: 'PENDING' },
      });
      if (existingApproval) {
      ...
      const requesterName = buildFullName(requester?.firstName ?? null, requester?.lastName ?? null);
      const objectLabel = targetObjectName === 'OPP' ? 'Opportunity' : 'Quote';
      createNotification({
      ...
      emailService.sendApprovalRequestEmail({
        ...
        objectType: targetObjectName === 'OPP' ? 'Opportunity' : 'Quote',
        objectName,
        objectNumber,
        approvalId: approval.id,
      }).catch((err) => console.error('[Approval] Failed to send approval request email:', err));
   ...
   * Approve or reject an approval request (ADMIN, SYSTEM_ADMIN only)
   * Body: { action: 'APPROVE' | 'REJECT', comment? }
   * Only the assigned approver (requestedTo) or a SYSTEM_ADMIN can act
   */
  async actionApproval(req: Request, res: Response) {
    const operation = 'Action on approval';
    try {
      const approvalId = this.parseId(req.params.id, res, 'Approval ID', operation);
      ...
      if (!action || !['APPROVE', 'REJECT'].includes(action)) {
        return handleValidationError(res, "action must be 'APPROVE' or 'REJECT'", 'action', operation);
      }
      ...
      if (approval.status !== 'PENDING') {
        return handleValidationError(res, `Approval is already ${approval.status}. Only PENDING approvals can be actioned.`, 'status', operation);
      }

      // Only the assigned approver or a SYSTEM_ADMIN can act
      if (approval.requestedToId !== userId && userRole !== UserRole.SYSTEM_ADMIN) {
        return res.status(403).json({ error: 'You are not authorized to action this approval. Only the assigned approver or a SYSTEM_ADMIN can act.' });
      }

      const newStatus: ApprovalStatus = action === 'APPROVE' ? 'APPROVED' : 'REJECTED';
      const actor = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, firstName: true, lastName: true } });

      let objectName = '';
      let objectNumber = '';
      let objectType: 'Opportunity' | 'Quote' = 'Opportunity';
        ...
        if (approval.targetObjectName === 'OPP') {
          ...
          const quote = await tx.quote.update({
            ...
            data: {
              ...
              ...(newStatus === 'APPROVED'
                ? { approvedAt: new Date(), approvedById: userId, approvalComment: comment || null, acceptedAt: new Date() }
                : { rejectedAt: new Date(), rejectedById: userId, rejectionComment: comment || null }),
            },
      ...
      }).catch((err) => console.error('[Approval] Failed to send approval action email:', err));
