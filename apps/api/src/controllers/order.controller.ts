...
export class OrderController {
    ...
    async createOrder(req: Request, res: Response) {
        try {
            ...
            if (!subdealerId) {
                return handleValidationError(res, 'Subdealer ID is required', 'subdealerId', 'Create Order');
            }

            if (!lineItems || !Array.isArray(lineItems) || lineItems.length === 0) {
                return handleValidationError(res, 'Line items are required', 'lineItems', 'Create Order');
            }
            ...
            if (!subdealer) {
                return handleValidationError(res, 'Subdealer not found', 'subdealerId', 'Create Order');
            }
            ...
            if (orderLineItems.length === 0) {
                return handleValidationError(res, 'No valid items in order', 'lineItems', 'Create Order');
            }
