...
export const integrationsController = {
  ...
  getConfig: async (_req: Request, res: Response) => {
    const keys = ['email.baseUrl', 'email.webhookSecret', 'whatsapp.baseUrl'] as const;
    ...
    res.json({
      ...
      whatsapp: {
        baseUrl: { exists: !!byKey['whatsapp.baseUrl'], value: byKey['whatsapp.baseUrl']?.plainValue ?? null },
      },
    });
  },

  upsertConfig: async (req: Request, res: Response) => {
    const body = req.body as Partial<{ email: { baseUrl?: string; webhookSecret?: string }; whatsapp: { baseUrl?: string } }>;
    const userId = String(req.user?.id ?? 'system');
    ...
    if (body.whatsapp?.baseUrl !== undefined) {
      const existingBaseUrl = await prisma.appConfig.findUnique({ where: { key: 'whatsapp.baseUrl' } });
      ...
      await prisma.appConfig.upsert({
        ...
        update: { plainValue: nextBaseUrl, encryptedValue: null, iv: null, authTag: null, maskedTail: null, updatedByUserId: userId },
        create: { key: 'whatsapp.baseUrl', plainValue: nextBaseUrl, updatedByUserId: userId },
      });
    }
