...
import { WhatsappSendService, CreateCampaignInput, UpdateCampaignInput } from '../services/whatsapp/SendService.js';
...
export class WhatsappController {
  ...
  createAccount = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Create account');
      }

      if (!validateRequiredFields(req.body, ['displayName', 'sourceHandle', 'apiKey'], res, 'Create account')) {
        return;
      }
      ...
      const record = await this.accountService.createAccount(
        {
          ...
          businessId: req.body.businessId ? String(req.body.businessId) : undefined,
  ...
  updateAccount = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Update account');
      }

      const accountId = Number(req.params.id);
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'Valid account ID is required', 'id', 'Update account');
      }

      if (!validateRequiredFields(req.body, ['displayName', 'status'], res, 'Update account')) {
        return;
      }
  ...
  listTemplates = async (req: Request, res: Response) => {
    try {
      ...
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'accountId is required', 'accountId', 'List templates');
      }
  ...
  syncTemplates = async (req: Request, res: Response) => {
    try {
      ...
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'accountId is required', 'accountId', 'Sync templates');
      }
  ...
  createTemplate = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Create template');
      }

      const accountId = Number(req.body.accountId);
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'accountId is required', 'accountId', 'Create template');
      }

      if (!validateRequiredFields(req.body, ['template_name', 'language', 'category', 'components'], res, 'Create template')) {
        return;
      }
      ...
      const result = await this.templateService.createTemplate(accountId, {
        ...
        message_ttl: req.body.message_ttl ? Number(req.body.message_ttl) : undefined,
        ttl_in_seconds: req.body.ttl_in_seconds ? Number(req.body.ttl_in_seconds) : undefined,
  ...
  updateTemplate = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Update template');
      }

      const accountId = Number(req.body.accountId);
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'accountId is required', 'accountId', 'Update template');
      }

      const templateName = req.params.name || req.body.template_name;
      if (!templateName) {
        return handleValidationError(res, 'template_name is required', 'template_name', 'Update template');
      }

      if (!req.body.components) {
        return handleValidationError(res, 'components is required', 'components', 'Update template');
      }
  ...
  uploadTemplateMedia = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Upload media');
      }

      const accountId = Number(req.body.accountId);
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'accountId is required', 'accountId', 'Upload media');
      }

      if (!req.body.mediaBase64 || !req.body.mimeType) {
        return handleValidationError(res, 'mediaBase64 and mimeType are required', 'media', 'Upload media');
      }
      ...
      const headerHandle = await this.templateService.uploadMedia(accountId, mediaBuffer, mimeType);
  ...
  uploadCampaignMedia = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Upload campaign media');
      }

      if (!req.body.mediaBase64 || !req.body.mimeType) {
        return handleValidationError(res, 'mediaBase64 and mimeType are required', 'media', 'Upload campaign media');
      }
  ...
  deleteTemplate = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Delete template');
      }

      const accountId = Number(req.query.accountId || req.body.accountId);
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'accountId is required', 'accountId', 'Delete template');
      }

      const templateName = req.params.name || req.query.name || req.body.template_name;
      if (!templateName) {
        return handleValidationError(res, 'template_name is required', 'template_name', 'Delete template');
      }

      const result = await this.templateService.deleteTemplate(accountId, String(templateName));
  ...
  syncNumbers = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Sync numbers');
      }
  ...
  getCampaignById = async (req: Request, res: Response) => {
    try {
      ...
      if (Number.isNaN(campaignId)) {
        return handleValidationError(res, 'campaign id must be a number', 'id', 'Get campaign');
      }
  ...
  getCampaignConfig = async (req: Request, res: Response) => {
    try {
      const campaignId = Number(req.params.id);
      if (Number.isNaN(campaignId)) {
        return handleValidationError(res, 'campaign id must be a number', 'id', 'Get campaign config');
      }

      const config = await this.sendService.getCampaignConfig(campaignId);
      if (!config) {
        return res.status(404).json({ error: 'Campaign configuration not found' });
      }
      res.json(config);
    } catch (error) {
  ...
  updateCampaign = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Update campaign');
      }

      const campaignId = Number(req.params.id);
      if (Number.isNaN(campaignId)) {
        return handleValidationError(res, 'campaign id must be a number', 'id', 'Update campaign');
      }
      ...
      const payload: UpdateCampaignInput = {
        ...
        audience: req.body.toAudience ? (String(req.body.toAudience).toLowerCase() as 'all' | 'segment') : undefined,
        segmentId: req.body.segmentId !== undefined ? (req.body.segmentId ? Number(req.body.segmentId) : null) : undefined,
      };
  ...
  createCampaign = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Create campaign');
      }

      if (!validateRequiredFields(req.body, ['name', 'accountId', 'templateName'], res, 'Create campaign')) {
        return;
      }

      const audience = String(req.body.toAudience || 'all').toLowerCase();
      if (!['all', 'segment', 'upload', 'leads'].includes(audience)) {
        return handleValidationError(res, 'Invalid audience value', 'toAudience', 'Create campaign');
      }

      const accountId = Number(req.body.accountId);
      if (Number.isNaN(accountId)) {
        return handleValidationError(res, 'accountId must be a number', 'accountId', 'Create campaign');
      }

      const segmentId = req.body.segmentId ? Number(req.body.segmentId) : undefined;
      if (segmentId !== undefined && Number.isNaN(segmentId)) {
        return handleValidationError(res, 'segmentId must be a number', 'segmentId', 'Create campaign');
      }
      ...
      if (payload.audience === 'segment' && !payload.segmentId) {
        return handleValidationError(res, 'segmentId is required for segment audience', 'segmentId', 'Create campaign');
      }

      if (payload.audience === 'upload') {
        if (!payload.csvContacts || !Array.isArray(payload.csvContacts) || payload.csvContacts.length === 0) {
          return handleValidationError(res, 'csvContacts is required for upload audience', 'csvContacts', 'Create campaign');
        }
        if (!payload.phoneColumnName) {
          return handleValidationError(res, 'phoneColumnName is required for upload audience', 'phoneColumnName', 'Create campaign');
        }
      }
  ...
  scheduleOrSend = async (req: Request, res: Response) => {
    try {
      const campaignId = Number(req.params.id);
      if (Number.isNaN(campaignId)) {
        return handleValidationError(res, 'campaign id must be a number', 'id', 'Send campaign');
      }
  ...
  scheduleCampaign = async (req: Request, res: Response) => {
    try {
      const campaignId = Number(req.params.id);
      if (Number.isNaN(campaignId)) {
        return handleValidationError(res, 'campaign id must be a number', 'id', 'Schedule campaign');
      }

      const { scheduledAt } = req.body;
      if (!scheduledAt) {
        return handleValidationError(res, 'scheduledAt is required', 'scheduledAt', 'Schedule campaign');
      }

      const scheduledDate = new Date(scheduledAt);
      if (scheduledDate <= new Date()) {
        return handleValidationError(res, 'Scheduled time must be in the future', 'scheduledAt', 'Schedule campaign');
      }

      const result = await this.sendService.scheduleCampaign(campaignId, scheduledDate);
  ...
  optOut = async (req: Request, res: Response) => {
    try {
      const { phone, channel = 'whatsapp', reason, source = 'manual' } = req.body;

      if (!phone) {
        return handleValidationError(res, 'Phone number is required', 'phone', 'Opt-out');
      }
  ...
  removeOptOut = async (req: Request, res: Response) => {
    try {
      ...
      if (!phone) {
        return handleValidationError(res, 'Phone number is required', 'phone', 'Remove opt-out');
      }

      const result = await this.optOutService.removeOptOut(phone, channel as 'whatsapp' | 'sms' | 'email');
  ...
  listOptOuts = async (req: Request, res: Response) => {
    try {
      ...
      const sortBy = (req.query.sortBy as 'optedOutAt' | 'phone') || 'optedOutAt';
  ...
  checkOptOut = async (req: Request, res: Response) => {
    try {
      ...
      if (!phone || typeof phone !== 'string') {
        return handleValidationError(res, 'Phone number is required', 'phone', 'Check opt-out');
      }

      const isOptedOut = await this.optOutService.isOptedOut(phone, channel as 'whatsapp' | 'sms' | 'email');
  ...
  listDeliveries = async (req: Request, res: Response) => {
    try {
      ...
      if (Number.isNaN(campaignId)) {
        return handleValidationError(res, 'campaignId is required', 'campaignId', 'List deliveries');
      }
  ...
  listEvents = async (req: Request, res: Response) => {
    try {
      const campaignId = Number(req.query.campaignId);
      if (Number.isNaN(campaignId)) {
        return handleValidationError(res, 'campaignId is required', 'campaignId', 'List events');
      }
  ...
  handleInboundMessage = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.webhookService.handleMsg91InboundMessage(req.body);
