...
export class OpportunityController {
  ...
  async createOpportunity(req: Request, res: Response) {
    ...
    try {
      ...
      if (!validateRequiredFields(req.body, ['name', 'accountId'], res, operation)) {
        return;
      }
      ...
      if (isNaN(parsedAccountId)) {
        return handleValidationError(res, 'Invalid accountId', 'accountId', operation);
      }
      ...
      if (contactId !== undefined && contactId !== null) {
        ...
        if (isNaN(parsedContactId)) {
          return handleValidationError(res, 'Invalid contactId', 'contactId', operation);
        }
      ...
      if (priceBookId !== undefined && priceBookId !== null) {
        ...
        if (isNaN(parsedPriceBookId)) {
          return handleValidationError(res, 'Invalid priceBookId', 'priceBookId', operation);
        }
      ...
      if (amount !== undefined && amount !== null) {
        ...
        if (isNaN(parsedAmount) || parsedAmount < 0) {
          return handleValidationError(res, 'Amount must be a non-negative number', 'amount', operation);
        }
      }
      ...
      if (expectedCloseDate) {
        ...
        if (isNaN(d.getTime())) {
          return handleValidationError(res, 'Invalid expectedCloseDate', 'expectedCloseDate', operation);
        }
  ...
  async getAllOpportunities(req: Request, res: Response) {
    ...
    try {
      ...
      if (createdFrom || createdTo) {
        ...
        if (Object.keys(createdAtFilter).length > 0) whereClause.createdAt = createdAtFilter;
      }
      if (expectedCloseFrom || expectedCloseTo) {
        ...
        if (Object.keys(expectedCloseFilter).length > 0) whereClause.expectedCloseDate = expectedCloseFilter;
      }
      if (amountMin !== undefined || amountMax !== undefined) {
        ...
        if (Object.keys(amountFilter).length > 0) whereClause.amount = amountFilter;
      }

      const allowedSortFields = ['createdAt', 'name', 'expectedCloseDate', 'stage', 'amount'];
      ...
      const data = opportunities.map((opp) => ({
        ...
        opportunityOwner: buildFullName(opp.owner.firstName, opp.owner.lastName),
   ...
   */
  private async generateQuoteNumber(opportunityId: number): Promise<{ quoteNumber: string; version: number }> {
    const now = new Date();
    const yy = String(now.getFullYear()).slice(-2);
    const mm = String(now.getMonth() + 1).padStart(2, '0');
  ...
  async generateQuote(req: Request, res: Response) {
    ...
    try {
      ...
      const userId = req.user!.id;
      const { validUntil, paymentTerms, deliveryTerms, notes, internalNotes } = req.body;
      ...
      if (opportunity.lineItems.length === 0) {
        return res.status(400).json({
          error: 'Opportunity has no line items. Add at least one line item before generating a quote.',
          code: 'NO_LINE_ITEMS',
        });
      }

      // 4. Generate quote number and version
      const { quoteNumber, version } = await this.generateQuoteNumber(opportunityId);
  ...
  async updateOpportunity(req: Request, res: Response) {
    ...
    try {
      const opportunityId = this.parseId(req.params.opportunityId, res, 'Opportunity ID', operation);
      if (opportunityId === null) return;
      ...
      const { name, expectedCloseDate, nextStep, type, stage, leadSource, priceBookId } = req.body;
      ...
      if (name !== undefined) {
        if (!name || typeof name !== 'string' || name.trim().length === 0) {
          return handleValidationError(res, 'Name cannot be empty', 'name', operation);
        }
        if (name.trim() !== existing.name) {
          activities.push({ field: 'name', oldValue: existing.name, newValue: name.trim() });
      ...
      if (expectedCloseDate !== undefined) {
        if (expectedCloseDate === null) {
          if (existing.expectedCloseDate !== null) {
            activities.push({ field: 'expectedCloseDate', oldValue: existing.expectedCloseDate?.toISOString() ?? null, newValue: null });
          ...
          activities.push({ field: 'expectedCloseDate', oldValue: existing.expectedCloseDate?.toISOString() ?? null, newValue: d.toISOString() });
      ...
      if (nextStep !== undefined) {
        ...
        if (newVal !== existing.nextStep) {
          activities.push({ field: 'nextStep', oldValue: existing.nextStep, newValue: newVal });
      ...
      if (type !== undefined) {
        if (type === null) {
          if (existing.type !== null) {
            activities.push({ field: 'type', oldValue: existing.type, newValue: null });
          ...
          if (type !== existing.type) {
            activities.push({ field: 'type', oldValue: existing.type, newValue: type });
      ...
      if (stage !== undefined) {
        ...
        if (stage !== existing.stage) {
          activities.push({ field: 'stage', oldValue: existing.stage, newValue: stage });
      ...
      if (leadSource !== undefined) {
        ...
        if (newVal !== existing.leadSource) {
          activities.push({ field: 'leadSource', oldValue: existing.leadSource, newValue: newVal });
      ...
      if (priceBookId !== undefined) {
        if (priceBookId === null) {
          if (existing.priceBookId !== null) {
            activities.push({ field: 'priceBookId', oldValue: String(existing.priceBookId), newValue: null });
          ...
          if (parsedPriceBookId !== existing.priceBookId) {
            ...
            activities.push({ field: 'priceBookId', oldValue: String(existing.priceBookId ?? ''), newValue: String(parsedPriceBookId) });
  ...
  async submitOpportunityForApproval(req: Request, res: Response) {
    ...
    try {
      ...
      if (opportunity.lineItems.length === 0) {
        return handleValidationError(res, 'Add at least one line item before submitting', 'lineItems', operation);
      }
      ...
      const threshold = thresholdSetting ? parseFloat(thresholdSetting.value) : 0;
      ...
      if (requiresApproval) {
        ...
        const existing = await prisma.approvalProcess.findFirst({
          where: { targetObjectName: 'OPP', targetRecordId: opportunityId, status: 'PENDING' },
        });
        if (existing) {
        ...
        }).catch((err) => console.error('[Opportunity] Failed to send approval email:', err));
  ...
  async addOpportunityLineItem(req: Request, res: Response) {
    ...
    try {
      ...
      const { productId, quantity: quantityParam, priceBookEntryId, listPrice: listPriceParam, discount: discountParam, description } = req.body;

      const parsedProductId = parseInt(productId, 10);
      if (isNaN(parsedProductId)) {
        return handleValidationError(res, 'Invalid productId', 'productId', operation);
      }
      ...
      if (priceBookEntryId !== undefined && priceBookEntryId !== null) {
        ...
        if (isNaN(parsedPriceBookEntryId)) {
          return handleValidationError(res, 'Invalid priceBookEntryId', 'priceBookEntryId', operation);
        }
        const priceBookEntry = await prisma.priceBookEntry.findUnique({
          where: { id: parsedPriceBookEntryId },
          select: { id: true, productId: true, listPrice: true, priceBookId: true },
        });
        if (!priceBookEntry) {
          return handleNotFoundError(res, 'Price book entry', operation);
        }
        if (priceBookEntry.productId !== parsedProductId) {
          return handleValidationError(res, 'Price book entry does not match product', 'priceBookEntryId', operation);
        }
        if (opportunity.priceBookId && priceBookEntry.priceBookId !== opportunity.priceBookId) {
          return handleValidationError(res, 'Price book entry does not belong to opportunity price book', 'priceBookEntryId', operation);
        }
        listPrice = Number(priceBookEntry.listPrice);
      } else {
        if (listPriceParam === undefined || listPriceParam === null) {
          return handleValidationError(res, 'listPrice is required when priceBookEntryId is not provided', 'listPrice', operation);
        }
        listPrice = parseFloat(listPriceParam);
        if (isNaN(listPrice) || listPrice < 0) {
          return handleValidationError(res, 'listPrice must be a non-negative number', 'listPrice', operation);
        }
      }

      const quantity = quantityParam !== undefined && quantityParam !== null
        ? Math.max(1, parseInt(quantityParam, 10) || 1)
        : 1;
      if (isNaN(quantity) || quantity < 1) {
        return handleValidationError(res, 'quantity must be a positive integer', 'quantity', operation);
      }

      const discount = discountParam !== undefined && discountParam !== null
        ? parseFloat(discountParam)
        : 0;
      if (isNaN(discount) || discount < 0 || discount > 100) {
        return handleValidationError(res, 'discount must be a number between 0 and 100', 'discount', operation);
      }
  ...
  async updateOpportunityLineItem(req: Request, res: Response) {
    ...
    try {
      ...
      const lineItemId = this.parseId(req.params.lineItemId, res, 'Line item ID', operation);
      ...
      const { quantity: quantityParam, listPrice: listPriceParam, discount: discountParam, description, sortOrder: sortOrderParam } = req.body;
