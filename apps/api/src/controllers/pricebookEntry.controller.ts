...
export class PriceBookEntryController {
  private parsePriceBookEntryId(id: string | undefined, res: Response, operation: string): number | null {
    if (!id) {
      handleValidationError(res, 'PriceBookEntry ID is required', 'id', operation);
      return null;
    }
  ...
  async getAllPriceBookEntries(req: Request, res: Response) {
    try {
        ...
        if (!productId) {
            return handleValidationError(res, 'productId is required', 'productId', 'Get all price book entries');
        }
  ...
  async getPriceBookEntryById(req: Request, res: Response) {
    try {
      const entryId = this.parsePriceBookEntryId(req.params.id, res, 'Get price book entry');
      ...
      if (!entry) {
        return handleNotFoundError(res, 'PriceBookEntry', 'Get price book entry');
      }
  ...
  async createPriceBookEntry(req: Request, res: Response) {
    try {
      const { priceBookId, productId, listPrice, useStandardPrice, isActive } = req.body;

      if (!priceBookId || !productId || listPrice === undefined) {
        return handleValidationError(res, 'Missing required fields: priceBookId, productId, and listPrice are required', undefined, 'Create price book entry');
      }
      
      const listPriceFloat = parseFloat(listPrice);
      if (isNaN(listPriceFloat)) {
        return handleValidationError(res, 'Invalid listPrice', 'listPrice', 'Create price book entry');
      }
  ...
  async updatePriceBookEntry(req: Request, res: Response) {
    try {
      const entryId = this.parsePriceBookEntryId(req.params.id, res, 'Update price book entry');
      if (entryId === null) return;
      ...
      if (listPrice !== undefined) {
        const listPriceFloat = parseFloat(listPrice);
        if (isNaN(listPriceFloat)) {
          return handleValidationError(res, 'Invalid listPrice', 'listPrice', 'Update price book entry');
        }
  ...
  async deletePriceBookEntry(req: Request, res: Response) {
    try {
      const entryId = this.parsePriceBookEntryId(req.params.id, res, 'Delete price book entry');
      if (entryId === null) return;
