...
const shouldSendCredentialEmail = (role: UserRole) =>
  role === UserRole.ADMIN || role === UserRole.SYSTEM_ADMIN;
...
export class UserController {
  ...
  async createUser(req: Request, res: Response) {
    try {
      const { firstName, lastName, email, phone, role, region, location } = req.body;
      ...
      const normalizedLocation = location
        ? location.trim().replace(/\s+/g, ' ').split(' ').map((word: string) =>
  ...
  async updateUser(req: Request, res: Response) {
    try {
      ...
      if (updateData.firstName !== undefined && !isValidName(updateData.firstName)) {
        return handleValidationError(
          res,
      ...
      if (updateData.lastName !== undefined && !isValidName(updateData.lastName)) {
        return handleValidationError(
          res,
  ...
  async importUsers(req: Request, res: Response) {
    try {
      ...
      if (!usersData || !Array.isArray(usersData) || usersData.length === 0) {
        return handleValidationError(res, 'Users data is required', 'users', 'Import users');
      }
      ...
      for (let i = 0; i < usersData.length; i++) {
        ...
        try {
          ...
          if (userData.region) {
            const normalizedRegion = userData.region.toUpperCase().replace(' ', '_');
          ...
          // Parse phone
          const parsedPhone = userData.phone ? parsePhoneNumber(userData.phone) : null;
          const countryCode = parsedPhone?.countryCode || '91';
          const localPhone = parsedPhone?.localNumber || userData.phone;

          // Normalize location
          const normalizedLocation = userData.location
            ? userData.location.trim().replace(/\s+/g, ' ').split(' ').map(word =>
                word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
              ).join(' ')
            : null;
          ...
          if (shouldSendCredentialEmail(role)) {
            ...
            emailService.sendUserCreationEmail({
              ...
              console.error(`Failed to send email to ${normalizedEmail}:`, error);
            });
          }
  ...
  async getImportTemplate(req: Request, res: Response) {
    res.json({
      columns: [
        ...
        { name: 'phone', label: 'Phone Number', required: false, format: '10 digits' },
        { name: 'email', label: 'Email', required: true },
        { name: 'role', label: 'Role', required: false, options: VALID_ROLES, default: 'SALES' },
        { name: 'region', label: 'Region', required: false, options: VALID_REGIONS, default: DEFAULT_REGION },
      ],
  ...
  async downloadTemplate(req: Request, res: Response) {
    try {
      ...
      instructionsSheet.addRows([
        ...
        { instruction: '  - Email: Valid email address (required, must be unique)' },
        { instruction: '' },
        ...
        { instruction: '  - Role and Region columns have dropdown lists for valid values' },
        { instruction: '  - A random password will be generated and emailed to each user' },
      ]);
  ...
  async downloadTemplateCsv(req: Request, res: Response) {
    try {
      const headers = ['First Name', 'Last Name', 'Email', 'Phone', 'Role', 'Region', 'Location'];
      const exampleRow = ['John', 'Doe', 'john.doe@example.com', '9876543210', 'SALES', 'SOUTH', 'Mumbai'];
      const instructions = [
        '# Instructions:',
   ...
   * Parse CSV file
   */
  private parseCsvFile(buffer: Buffer): { headerMap: Record<string, number>; rows: any[][] } {
  ...
  async importUsersFile(req: Request, res: Response) {
    try {
      ...
      if (!file) {
        return handleValidationError(res, 'No file uploaded', 'file', 'Import users');
      }
      ...
      if (fileType === 'csv') {
        ...
        if (!ws) {
          return handleValidationError(res, 'Empty workbook', 'file', 'Import users');
        }

        const headerRow = ws.getRow(1);
        headerRow.eachCell((cell, colNumber) => {
          const header = String(cell.value || '').trim().toLowerCase().replace(/\s+/g, '');
        ...
        for (let r = 2; r <= ws.rowCount; r++) {
          ...
          row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
            let val = cell.value;
            if (val == null) val = '';
            else if (typeof val === 'object' && 'text' in val) val = (val as any).text;
      ...
      const phoneCol = getCol(['phone', 'phonenumber', 'phone number', 'mobile']);
      ...
      const results = {
        success: [] as any[],
        errors: [] as { row: number; firstName: string; lastName: string; email: string; phone: string; role: string; region: string; location: string; error: string }[],
      };
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const role = roleCol >= 0 ? (row[roleCol] || 'SALES').trim().toUpperCase() : 'SALES';
        const region = regionCol >= 0 ? (row[regionCol] || '').trim().toUpperCase().replace(' ', '_') : '';
        // Normalize location: trim, remove extra spaces, and capitalize each word
        const rawLocation = locationCol >= 0 ? (row[locationCol] || '').trim() : '';
        const location = rawLocation
          ? rawLocation.replace(/\s+/g, ' ').split(' ').map((word: string) =>
              word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
            ).join(' ')
          : '';

        const errorRow = { row: rowNum, firstName, lastName, email, phone, role, region, location, error: '' };
      ...
      let report: { filename: string; mimeType: string; base64: string } | undefined;
