  ...
  try {
    ...
    for (const k of Object.keys(obj || {})) {
      ...
      if (v == null) {
        ...
        clone[k] = v.length > maxLen ? `${v.slice(0, maxLen)}... [truncated]` : v;
      } else if (typeof v === "object") {
        // shallow stringify, truncated
        const str = JSON.stringify(v);
        clone[k] = str.length > maxLen ? `${str.slice(0, maxLen)}... [truncated]` : JSON.parse(str);
      } else {
        clone[k] = v;
      }
    }
...
export class WebhookController {
  async handleLandingiWebhook(req: Request, res: Response) {
    try {
      ...
      if (secret) {
        const signatureHeader = req.get("x-landingi-signature") || req.get("x-hub-signature-256") || req.get("x-signature");
        if (!signatureHeader) {
          console.warn("⚠️ Webhook signature header missing but secret is configured");
        ...
        if (!crypto.timingSafeEqual(Buffer.from(computed), Buffer.from(signatureHeader))) {
      ...
      try {
        ...
        if (leadData) {
          ...
          const campaignUniqueId = this.extractLandingPageCampaignId(webhookData);
          const customFields = this.extractCustomFields(webhookData);

          createdLead = await this.storeLeadWithEnquiry(leadData, campaignUniqueId, customFields);
        ...
        console.error("❌ Error storing lead and enquiry (masked):", {
          message: leadError instanceof Error ? leadError.message : String(leadError),
        });
      ...
      try {
        const formSubmissionData = this.extractFormSubmissionData(webhookData, createdLead);
        if (formSubmissionData) {
          createdFormSubmission = await this.storeFormSubmission(formSubmissionData);
          console.log("✅ Form submission stored (id):", createdFormSubmission?.id);
        } else {
        ...
        console.error("❌ Error storing form submission:", {
          message: formSubmissionError instanceof Error ? formSubmissionError.message : String(formSubmissionError),
        });
      }
      ...
      console.error("❌ Unhandled error processing Landingi webhook:", error instanceof Error ? error.message : String(error));
      handleError(error, res, "Process Landingi webhook");
    }
  }

  async testLandingiWebhook(req: Request, res: Response) {
    console.log("🧪 Landingi Webhook Test Endpoint Accessed", new Date().toISOString());
    return res.json({
      success: true,
      message: "Landingi webhook test endpoint is working",
      timestamp: new Date().toISOString(),
      webhookUrl: `${req.protocol}://${req.get("host")}/api/webhook/landingi`,
      instructions: "Send POST requests to the webhook URL to test the webhook receiver",
    });
  }
  ...
  private extractCustomFields(webhookData: any): any | null {
    try {
      ...
      for (const key of Object.keys(webhookData || {})) {
        if (!["form_submission", "custom_fields", "lead", "campaign", "landing_page"].includes(key) && !contactFields.includes(key.toLowerCase())) {
  ...
  private extractLeadFromDirectFields(webhookData: any): any | null {
    ...
    if (webhookData.phone || webhookData.phone_number || webhookData.telephone) {
      leadData.phone = normalizePhone(
  ...
  private async storeLeadWithEnquiry(leadData: any, campaignUniqueId: string | null, customFields: any): Promise<any> {
    try {
      // Basic validations
      ...
      if (campaignUniqueId) {
        ...
        if (!landingPageCampaign) {
          console.warn(`⚠️ Landing page campaign not found with uniqueId: ${campaignUniqueId}`);
        }
      }

      if (existingLead) {
        console.log("📧 Existing lead found, creating enquiry for lead id:", existingLead.id);
        lead = existingLead;
      ...
      console.error("Database error storing lead and enquiry:", error instanceof Error ? error.message : String(error));
      throw error;
    }
  }

  private extractFormSubmissionData(webhookData: any, createdLead: any): any | null {
    try {
      ...
      const formSubmissionData = {
        ...
        formData: {
          webhookSummary: {
            ...
            sample: {
              email: maskEmail(
                webhookData.form_submission?.email || webhookData.email || webhookData.custom_fields?.email
              ),
              phone: maskPhone(
                webhookData.form_submission?.phone || webhookData.phone || webhookData.custom_fields?.phone
              ),
            },
          },
