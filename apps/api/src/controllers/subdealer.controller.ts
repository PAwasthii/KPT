...
export class SubdealerController {
  ...
  async generateOtp(req: Request, res: Response) {
    try {
      ...
      if (!otpSent) {
        return handleError(new Error('Failed to send OTP via MSG91'), res, 'Generate OTP');
      }
  ...
  async checkPhone(req: Request, res: Response) {
    try {
      ...
      if (!phone) {
        return handleValidationError(res, 'Phone number is required', 'phone', 'Check Phone');
      }
  ...
  async login(req: Request, res: Response) {
    try {
      ...
      // Validate phone number
      if (!isValidPhone(normalizedPhone)) {
        return handleValidationError(res, 'Invalid phone number', 'phone', 'Login');
      }
