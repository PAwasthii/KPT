...
import { SegmentService, SegmentPayload, SegmentRuleInput, SegmentRuleOperator } from '../services/segment.service.js';
import {
  handleError,
  handleNotFoundError,
...
import { SegmentRuleType, SegmentEntityType, AuditCategory } from '@prisma/client';
...
export class SegmentController {
  ...
  getById = async (req: Request, res: Response) => {
    try {
      ...
      if (Number.isNaN(id)) {
        return handleValidationError(res, 'Segment id must be a number', 'id', 'Get segment');
      }
  ...
  create = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Create segment');
      }
      ...
      if (!payload) {
        return handleValidationError(res, 'Invalid rules payload', 'rules', 'Create segment');
      }
      ...
      await recordAuditLog({
        ...
        entityType: 'Segment',
        entityId: segment.id,
        newValues: { name: segment.name, logicOperator: segment.logicOperator, rules: payload.rules },
  ...
  update = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Update segment');
      }

      const id = Number(req.params.id);
      if (Number.isNaN(id)) {
        return handleValidationError(res, 'Segment id must be a number', 'id', 'Update segment');
      }
      ...
      const payload = this.normalizePayload(req.body);
      if (!payload) {
        return handleValidationError(res, 'Invalid rules payload', 'rules', 'Update segment');
      }

      const segment = await this.service.updateSegment(id, payload, req.user.id);
      ...
      await recordAuditLog({
        ...
        entityType: 'Segment',
        entityId: segment.id,
        oldValues: { name: existing.name, logicOperator: existing.logicOperator, rules: existing.rules },
        newValues: { name: segment.name, logicOperator: segment.logicOperator, rules: payload.rules },
        category: AuditCategory.CAMPAIGN_MANAGEMENT,
        subCategory: WHATSAPP_SUBCATEGORY,
      });
  ...
  remove = async (req: Request, res: Response) => {
    try {
      if (!req.user?.id) {
        return handleValidationError(res, 'Authenticated user required', 'user', 'Delete segment');
      }

      const id = Number(req.params.id);
      if (Number.isNaN(id)) {
        return handleValidationError(res, 'Segment id must be a number', 'id', 'Delete segment');
      }
      ...
      await recordAuditLog({
        ...
        entityType: 'Segment',
        entityId: id,
        oldValues: { name: existing.name, logicOperator: existing.logicOperator },
        category: AuditCategory.CAMPAIGN_MANAGEMENT,
        subCategory: WHATSAPP_SUBCATEGORY,
      });
  ...
  resolve = async (req: Request, res: Response) => {
    try {
      const id = Number(req.params.id);
      if (Number.isNaN(id)) {
        return handleValidationError(res, 'Segment id must be a number', 'id', 'Resolve segment');
      }

      const limitSource = req.query.limit ?? (req.body?.limit as number | string | undefined);
      const limit = limitSource ? Number(limitSource) : 200;
      if (Number.isNaN(limit) || limit < 0 || limit > 1000) {
        return handleValidationError(res, 'limit must be between 0 and 1000', 'limit', 'Resolve segment');
      }
  ...
  private normalizePayload(body: Record<string, unknown>): SegmentPayload | null {
    const logicValue = body.logicOperator;
    const logicOperator = typeof logicValue === 'string' ? logicValue.toUpperCase() : 'AND';
    ...
    for (let i = 0; i < rawRules.length; i += 1) {
      ...
      const type = typeof raw?.ruleType === 'string' ? raw.ruleType.toUpperCase() : '';
      if (!Object.values(SegmentRuleType).includes(type as SegmentRuleType)) {
        return null;
      }

      const operator = typeof raw?.operator === 'string' ? raw.operator.toUpperCase() : 'IN';
