 ...
 */
function generateSalesUserToken(userId: number, phone: string, email: string): string {
  return jwt.sign(
    { userId, phone, email, type: 'sales_user' },
    JWT_SECRET,
    { expiresIn: '7d' }
  );
}
 ...
 */
export function verifySalesUserToken(token: string): { userId: number; phone: string; email: string; type: string } | null {
  try {
    const decoded = jwt.verify(token, JWT_SECRET) as { userId: number; phone: string; email: string; type: string };
...
export class AakramanController {
  ...
  async sendSmsOtp(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!user) {
        res.status(404).json({ error: 'No sales user found with this phone number' });
        return;
      }
  ...
  async verifyOtp(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Generate JWT token
      const token = generateSalesUserToken(user.id, user.phone || '', user.email);
  ...
  async createOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { firmName, ownerFirstName, ownerLastName, contactNumber, email, city, state, pincode, gst } = firmDetails;

      if (!firmName || !ownerFirstName || !ownerLastName || !contactNumber || !city || !state) {
        res.status(400).json({ error: 'Required firm details: firmName, ownerFirstName, ownerLastName, contactNumber, city, state' });
        return;
      }
  ...
  async updateOrder(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { status, firmName, ownerFirstName, ownerLastName, contactNumber, email, city, state, pincode } = req.body;
  ...
  /**
   * Archive an order (SYSTEM_ADMIN only)
  ...
  /**
   * Unarchive an order (SYSTEM_ADMIN only)
  ...
  async unarchiveOrder(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.json({ success: true, message: 'Order unarchived successfully', order });
    } catch (error) {
