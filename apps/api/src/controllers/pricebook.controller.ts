...
export class PriceBookController {
  private parsePriceBookId(id: string | undefined, res: Response, operation: string): number | null {
    if (!id) {
  ...
  async getAllPriceBooks(req: Request, res: Response) {
    try {
      ...
      if (isNaN(pageNum) || isNaN(limitNum)) {
        return handleValidationError(res, 'page and limit must be numbers', undefined, 'Get all price books');
      }
  ...
  async getPriceBookById(req: Request, res: Response) {
    try {
      const pricebookId = this.parsePriceBookId(req.params.id, res, 'Get price book');
  ...
  async createPriceBook(req: Request, res: Response) {
    try {
      ...
      if (!name) {
        return handleValidationError(res, 'Name is required', 'name', 'Create price book');
      }
  ...
  async updatePriceBook(req: Request, res: Response) {
    try {
      const pricebookId = this.parsePriceBookId(req.params.id, res, 'Update price book');
      if (pricebookId === null) return;
  ...
  async deletePriceBook(req: Request, res: Response) {
    try {
      const pricebookId = this.parsePriceBookId(req.params.id, res, 'Delete price book');
      if (pricebookId === null) return;
