...
export class LeadController {
  ...
  async getAllLeads(req: Request, res: Response) {
    try {
      ...
      if (keywordIds) {
        const keywordIdsArray = keywordIds.toString().split(",").map((id: string) => parseInt(id.trim())).filter((id: number) => !isNaN(id));
    ...
    try {
      ...
      if (
        ...
        return handleForbiddenError(
          res,
          "Only admins can view assignment statistics",
          "Get assignment stats"
        );
      }
  ...
  async updateLead(req: Request, res: Response) {
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(updateData, "ownerId")) {
        ...
        (updateData as any).ownerId = rawOwnerId === "" || rawOwnerId == null
          ? null
      ...
      if (updateData.pincode !== undefined && updateData.pincode && !isValidPincode(updateData.pincode)) {
        return handleValidationError(
          res,
          "Invalid pincode. Pincode must be exactly 6 digits",
          "pincode",
      ...
      if (updateData.companyName !== undefined && updateData.companyName && !validateFieldLength(updateData.companyName, 255)) {
        return handleValidationError(
          res,
          "Company name must be 255 characters or less",
          "companyName",
      ...
      if (updateData.city !== undefined && updateData.city && !validateFieldLength(updateData.city, 100)) {
        return handleValidationError(
          res,
          "City must be 100 characters or less",
          "city",
      ...
      if (updateData.state !== undefined && updateData.state && !validateFieldLength(updateData.state, 100)) {
        return handleValidationError(
          res,
          "State must be 100 characters or less",
          "state",
      ...
      if (Object.prototype.hasOwnProperty.call(filteredData, "status")) {
        ...
        if (currentStatusStr === convertedStatusStr && newStatusStr !== convertedStatusStr) {
          filteredData.convertedToContactId = null;
          console.log("🔄 Clearing convertedToContactId: status changed from CONVERTED to", newStatus);
          console.log("🔄 Current convertedToContactId:", currentLead.convertedToContactId);
        }
      }
      ...
      console.log(JSON.stringify(lead, null, 2));
      console.log("🔄 Updated lead convertedToContactId:", lead.convertedToContactId);
  ...
  async assignLeadToUser(req: Request, res: Response) {
    try {
      ...
      const previousOwnerName = result.previousOwner ? `${result.previousOwner.firstName || ''} ${result.previousOwner.lastName || ''}`.trim() : "Unassigned";
      const newOwnerName = `${result.newOwner.firstName || ''} ${result.newOwner.lastName || ''}`.trim();
  ...
  async assignLeadsBulkToUser(req: Request, res: Response) {
    try {
      ...
      const result = await prisma.$transaction(async tx => {
        ...
        console.log(`Assigning ${leadIds.length} leads to user: ${userFullName}`);
      ...
      const resultUserFullName = `${result.user.firstName || ''} ${result.user.lastName || ''}`.trim();
      const summary = {
  ...
  async claimLead(req: Request, res: Response) {
    try {
      ...
      const result = await prisma.$transaction(async tx => {
        ...
        const currentOwnerName = lead.owner ? `${lead.owner.firstName || ''} ${lead.owner.lastName || ''}`.trim() : "Unclaimed";
        console.log("Lead found:", {
          id: lead.id,
        ...
        const newOwnerName = updatedLead.owner ? `${updatedLead.owner.firstName || ''} ${updatedLead.owner.lastName || ''}`.trim() : '';
      ...
      const claimedByUserName = `${result.claimDetails.claimedByUser.firstName || ''} ${result.claimDetails.claimedByUser.lastName || ''}`.trim();
      ...
      for (const leadId of leadIds) {
        ...
        try {
          ...
          const result = await prisma.$transaction(async tx => {
            ...
            const leadDisplayName = getLeadDisplayName(lead);
            const currentOwnerNameBulk = lead.owner ? `${lead.owner.firstName || ''} ${lead.owner.lastName || ''}`.trim() : "Unclaimed";
            console.log("Lead found:", {
              id: lead.id,
              name: leadDisplayName,
              email: lead.email,
            ...
            const bulkUserFullName = `${user.firstName || ''} ${user.lastName || ''}`.trim();
            console.log("User found:", {
              id: user.id,
            ...
            const updatedLeadName = getLeadDisplayName(updatedLead);
            const bulkNewOwnerName = updatedLead.owner ? `${updatedLead.owner.firstName || ''} ${updatedLead.owner.lastName || ''}`.trim() : '';
            console.log("Lead claimed successfully:", {
              leadId: updatedLead.id,
              leadName: updatedLeadName,
