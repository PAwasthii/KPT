...
export class QuoteController {
  ...
  async getAllQuotes(req: Request, res: Response) {
    try {
      ...
      const { status, opportunityId, accountId, isPrimary, sortBy, sortOrder } = req.query;
      ...
      // Build orderBy
      const allowedSortFields = ['createdAt', 'quoteNumber', 'grandTotal', 'status', 'validUntil'];
  ...
  /**
   * PATCH /api/quotes/:id
   * Update quote status (ADMIN, SYSTEM_ADMIN only)
   */
   ...
   * Set a quote as primary (ADMIN, SYSTEM_ADMIN only)
  ...
  async setPrimaryQuote(req: Request, res: Response) {
    ...
    try {
      ...
      if (quote.isPrimary) {
        return handleValidationError(res, 'This quote is already the primary quote', 'isPrimary', operation);
      }
   ...
   * Generate a sales order from an accepted quote (ADMIN, SYSTEM_ADMIN only)
  ...
  async generateOrder(req: Request, res: Response) {
    ...
    try {
      ...
      if (quote.status !== 'ACCEPTED') {
        return handleValidationError(
          res,
          'Only accepted quotes can generate orders. Current status: ' + quote.status,
          'status',
          operation
        );
      }
  ...
  async getQuoteLineItems(req: Request, res: Response) {
    ...
    try {
      ...
      // Execute count query
      const totalItems = await prisma.quoteLineItem.count({ where: whereClause });
  ...
  async generatePdf(req: Request, res: Response) {
    ...
    try {
      ...
      doc.moveDown(0.5);
      doc.fontSize(12).font('Helvetica').text(quote.quoteNumber, { align: 'center' });
      doc.moveDown(1);
      ...
      doc.text(`Valid Until: ${quote.validUntil ? new Date(quote.validUntil).toLocaleDateString() : 'N/A'}`, 50);

      // Right column
      doc.text(`Date: ${new Date(quote.createdAt).toLocaleDateString()}`, 300, infoStartY);
      ...
      doc.moveDown(0.3);
      doc.font('Helvetica').fontSize(9);
      doc.text(`${quote.preparedBy.firstName || ''} ${quote.preparedBy.lastName || ''}`.trim(), 300);
      ...
      if (quote.billingName || quote.shippingName) {
        ...
        if (quote.billingName) {
          doc.fontSize(10).font('Helvetica-Bold').text('Billing Address', 50, addrY);
          doc.moveTo(50, doc.y).lineTo(290, doc.y).stroke();
          doc.moveDown(0.3);
          doc.font('Helvetica').fontSize(9);
          if (quote.billingName) doc.text(quote.billingName, 50);
          if (quote.billingStreet) doc.text(quote.billingStreet, 50);
          const billingCityLine = [quote.billingCity, quote.billingState, quote.billingPostalCode].filter(Boolean).join(', ');
        ...
        if (quote.shippingName) {
          doc.fontSize(10).font('Helvetica-Bold').text('Shipping Address', 300, addrY);
          doc.moveTo(300, doc.y).lineTo(545, doc.y).stroke();
          doc.moveDown(0.3);
          doc.font('Helvetica').fontSize(9);
          if (quote.shippingName) doc.text(quote.shippingName, 300);
          if (quote.shippingStreet) doc.text(quote.shippingStreet, 300);
          const shippingCityLine = [quote.shippingCity, quote.shippingState, quote.shippingPostalCode].filter(Boolean).join(', ');
      ...
      const colX = { num: 50, product: 70, qty: 280, listPrice: 320, unitPrice: 380, discount: 440, total: 490 };
      ...
      doc.text('List Price', colX.listPrice, tableTop, { width: 55, align: 'right' });
      doc.text('Unit Price', colX.unitPrice, tableTop, { width: 55, align: 'right' });
      doc.text('Disc %', colX.discount, tableTop, { width: 40, align: 'right' });
      ...
      quote.lineItems.forEach((item, index) => {
        ...
        doc.text(item.product?.name || `Product #${item.productId}`, colX.product, currentY, { width: 200 });
        doc.text(`${item.quantity}`, colX.qty, currentY, { width: 40, align: 'right' });
        doc.text(`${Number(item.listPrice).toFixed(2)}`, colX.listPrice, currentY, { width: 55, align: 'right' });
        doc.text(`${Number(item.unitPrice).toFixed(2)}`, colX.unitPrice, currentY, { width: 55, align: 'right' });
        doc.text(`${Number(item.discount).toFixed(2)}`, colX.discount, currentY, { width: 40, align: 'right' });
        doc.text(`${Number(item.totalPrice).toFixed(2)}`, colX.total, currentY, { width: 55, align: 'right' });
        doc.moveDown(0.5);
      });
      ...
      doc.text(`${Number(quote.subtotal).toFixed(2)}`, summaryValX, doc.y - doc.currentLineHeight(), { width: summaryWidth, align: 'right' });
      doc.moveDown(0.3);

      if (Number(quote.discount) > 0) {
        doc.text(`Discount (${Number(quote.discountPercent).toFixed(2)}%):`, summaryX, doc.y, { width: 100 });
        doc.text(`-${Number(quote.discount).toFixed(2)}`, summaryValX, doc.y - doc.currentLineHeight(), { width: summaryWidth, align: 'right' });
        doc.moveDown(0.3);
      }

      if (Number(quote.taxAmount) > 0) {
        doc.text(`Tax (${Number(quote.taxPercent).toFixed(2)}%):`, summaryX, doc.y, { width: 100 });
        doc.text(`${Number(quote.taxAmount).toFixed(2)}`, summaryValX, doc.y - doc.currentLineHeight(), { width: summaryWidth, align: 'right' });
        doc.moveDown(0.3);
      }

      if (Number(quote.shippingAmount) > 0) {
        doc.text('Shipping:', summaryX, doc.y, { width: 100 });
        doc.text(`${Number(quote.shippingAmount).toFixed(2)}`, summaryValX, doc.y - doc.currentLineHeight(), { width: summaryWidth, align: 'right' });
        doc.moveDown(0.3);
      }
      ...
      doc.text(`${Number(quote.grandTotal).toFixed(2)}`, summaryValX, doc.y - doc.currentLineHeight(), { width: summaryWidth, align: 'right' });
      doc.moveDown(1.5);
   ...
   * Submit a DRAFT quote for internal approval (ADMIN, SYSTEM_ADMIN only)
  ...
  async submitForApproval(req: Request, res: Response) {
    ...
    try {
      ...
      if (!requestedToId) {
        return handleValidationError(res, 'requestedToId is required', 'requestedToId', operation);
      }

      const parsedRequestedToId = parseInt(requestedToId, 10);
      if (isNaN(parsedRequestedToId)) {
        return handleValidationError(res, 'Invalid requestedToId', 'requestedToId', operation);
      }
      ...
      const approver = await prisma.user.findUnique({ where: { id: parsedRequestedToId } });
      if (!approver) {
        return handleNotFoundError(res, 'Approver user', operation);
      }
      if (approver.role === UserRole.SALES) {
        return handleValidationError(res, 'Approver must have ADMIN or SYSTEM_ADMIN role', 'requestedToId', operation);
      }

      // Check for existing pending approval
      const existing = await prisma.approvalProcess.findFirst({
        where: { targetObjectName: 'QUOTE', targetRecordId: quoteId, status: 'PENDING' },
      });
      if (existing) {
      ...
      const approval = await prisma.$transaction(async (tx) => {
        const newApproval = await tx.approvalProcess.create({
          ...
          include: {
            requestedTo: { select: { id: true, firstName: true, lastName: true, email: true } },
      ...
      emailService.sendApprovalRequestEmail({
        ...
        requesterName: buildFullName(requester?.firstName ?? null, requester?.lastName ?? null),
        objectType: 'Quote',
        objectName: quote.name,
        objectNumber: quote.quoteNumber,
        approvalId: approval.id,
      }).catch((err) => console.error('[Quote] Failed to send approval request email:', err));
   ...
   * Send an APPROVED quote to the client via email (ADMIN, SYSTEM_ADMIN only)
  ...
  async sendToClient(req: Request, res: Response) {
    ...
    try {
      ...
      if (!to || typeof to !== 'string' || !to.includes('@')) {
        return handleValidationError(res, 'A valid recipient email (to) is required', 'to', operation);
      }
      ...
      const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
        ...
        doc.text(`Version: ${quote.version}`, 50);
        doc.text(`Valid Until: ${quote.validUntil ? new Date(quote.validUntil).toLocaleDateString() : 'N/A'}`, 50);
        doc.text(`Date: ${new Date(quote.createdAt).toLocaleDateString()}`, 300, infoY);
        doc.text(`Opportunity: ${quote.opportunity.name}`, 300);
        ...
        doc.moveDown(0.3);
        const tableTop = doc.y;
        const colX = { num: 50, product: 70, qty: 280, unitPrice: 330, discount: 410, total: 470 };
        doc.fontSize(8).font('Helvetica-Bold');
        doc.text('#', colX.num, tableTop, { width: 20 });
        doc.text('Product', colX.product, tableTop, { width: 200 });
        doc.text('Qty', colX.qty, tableTop, { width: 45, align: 'right' });
        doc.text('Unit Price', colX.unitPrice, tableTop, { width: 70, align: 'right' });
        doc.text('Disc %', colX.discount, tableTop, { width: 50, align: 'right' });
        ...
        quote.lineItems.forEach((item, index) => {
          ...
          doc.text(item.product?.name || `Product #${item.productId}`, colX.product, rowY, { width: 200 });
          doc.text(`${item.quantity}`, colX.qty, rowY, { width: 45, align: 'right' });
          doc.text(`${Number(item.unitPrice).toFixed(2)}`, colX.unitPrice, rowY, { width: 70, align: 'right' });
          doc.text(`${Number(item.discount).toFixed(2)}`, colX.discount, rowY, { width: 50, align: 'right' });
          doc.text(`${Number(item.totalPrice).toFixed(2)}`, colX.total, rowY, { width: 75, align: 'right' });
          doc.moveDown(0.5);
        });
        doc.moveTo(50, doc.y).lineTo(545, doc.y).stroke();
        doc.moveDown(0.5);
        ...
        doc.text(`${Number(quote.subtotal).toFixed(2)}`, sumValX, doc.y - doc.currentLineHeight(), { width: 75, align: 'right' });
        doc.moveDown(0.3);
        if (Number(quote.discount) > 0) {
          doc.text(`Discount (${Number(quote.discountPercent).toFixed(2)}%):`, sumX, doc.y, { width: 85 });
          doc.text(`-${Number(quote.discount).toFixed(2)}`, sumValX, doc.y - doc.currentLineHeight(), { width: 75, align: 'right' });
          doc.moveDown(0.3);
        }
        if (Number(quote.taxAmount) > 0) {
          doc.text(`Tax (${Number(quote.taxPercent).toFixed(2)}%):`, sumX, doc.y, { width: 85 });
          doc.text(`${Number(quote.taxAmount).toFixed(2)}`, sumValX, doc.y - doc.currentLineHeight(), { width: 75, align: 'right' });
          doc.moveDown(0.3);
        }
        if (Number(quote.shippingAmount) > 0) {
          doc.text('Shipping:', sumX, doc.y, { width: 85 });
          doc.text(`${Number(quote.shippingAmount).toFixed(2)}`, sumValX, doc.y - doc.currentLineHeight(), { width: 75, align: 'right' });
          doc.moveDown(0.3);
        }
        ...
        doc.text(`${Number(quote.grandTotal).toFixed(2)}`, sumValX, doc.y - doc.currentLineHeight(), { width: 75, align: 'right' });
        doc.moveDown(1.5);
        ...
        doc.fontSize(8).font('Helvetica').fillColor('gray');
        doc.text(`Generated on ${new Date().toLocaleString()}`, 50, doc.page.height - 50, { align: 'center', width: 495 });
        doc.end();
      });
  ...
  async getQuotesByOpportunityId(req: Request, res: Response) {
    ...
    try {
      const opportunityId = this.parseId(req.params.id, res, 'Opportunity ID', operation);
