...
export async function getMsg91Credentials(): Promise<Msg91Credentials> {
  ...
  if (!credential) {
    throw new Error('MSG91 API key not configured. Please configure it in the integration manager.');
  }

  const apiKey = decryptSecret(credential.encryptedApiKey, credential.iv, credential.authTag);
  ...
  const baseUrl = baseUrlConfig?.plainValue || process.env.MSG91_BASE_URL || 'https://control.msg91.com/api/v5';
