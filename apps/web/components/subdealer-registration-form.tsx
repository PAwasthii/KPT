    ...
    if (newPhone.length === 10 && previousLength !== 10) {
      ...
      if (result.success) {
        ...
        try {
          ...
          setIsExistingUser(false);
          toast.error("Could not verify phone number. Please continue with registration.");
          setTimeout(() => setStep(2), 100);
        } finally {
          setLoading(false);
        }
  ...
  if (step === 5 && success) {
    return (
      <PostVerificationFlow
        ...
        phone={phone}
        subdealerId={existingUserData?.id || (localStorage.getItem('subdealer_token') ? JSON.parse(atob(localStorage.getItem('subdealer_token')!.split('.')[1] || '')).id : null)}
      />
    );
  }
                ...
                {step === 4 && (
                  ...
                  >
                    {loading ? (
                      ...
                      isExistingUser ? "Verify OTP & Login" : "Verify OTP & Register"
                    )}
                  </Button>
                )}
              </Field>
...
import { FileUp, ShoppingCart, Plus, Minus, ArrowRight, Search, Trash2, CheckCircle } from "lucide-react";
import { invoiceService, orderService, productService } from "@/lib/api/services";
import { Product } from "@/lib/api/types";

function PostVerificationFlow({ gstDetails, phone, subdealerId }: { gstDetails: GstDetails | null, phone: string, subdealerId: number | null }) {
      ...
      <div className="container mx-auto max-w-md px-4 py-8">
        <Card className="w-full">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              Welcome, {gstDetails?.legalName || 'Subdealer'}. What would you like to do next?
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            >
              ...
              <span className="text-xs text-muted-foreground">Upload your past invoices</span>
            </Button>
            <Button
              className="w-full h-auto py-4 flex flex-col items-center gap-2"
            ...
            >
              ...
              <span className="text-xs text-white/80">Browse catalog and place order</span>
            </Button>
          </CardContent>
        </Card>
      </div>
  ...
  if (mode === 'order') {
    return <OrderBookingView subdealerId={subdealerId} onBack={() => setMode('choice')} />;
  }

  return null;
}

function InvoiceUploadView({ phone, onBack }: { phone: string, onBack: () => void }) {
    ...
    <div className="container mx-auto max-w-md px-4 py-8">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Upload your invoice file (PDF, JPG, PNG)</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={onBack} disabled={uploading}>Back</Button>
...
function OrderBookingView({ subdealerId, onBack }: { subdealerId: number | null, onBack: () => void }) {
  ...
  const [orderSuccess, setOrderSuccess] = useState<{ id: number, orderNumber: string, totalAmount: string | number } | null>(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>Are you sure you want to place this order?</CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="flex gap-3">
                <Button variant="outline" className="flex-1" onClick={() => setShowConfirmation(false)} disabled={submitting}>
                  Cancel
                </Button>
                <Button className="flex-1" onClick={confirmOrder} disabled={submitting}>
                  {submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Confirm Order"}
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="sticky top-4">
            ...
            <CardContent className="space-y-6">
              {Object.keys(cart).length === 0 ? (
                ...
                <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                  {Object.entries(cart).map(([pid, qty]) => {
                    ...
                    return (
                      <div key={pid} className="flex gap-3 items-start pb-4 border-b last:border-0 last:pb-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            <input
                              type="number"
                              min={1}
                              className="h-8 w-16 border rounded-md px-2 text-sm"
                              value={qty}
                              onChange={(e) => {
              ...
              <div className="pt-4 border-t space-y-4">
                ...
                >
                  {submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Place Order"}
                </Button>
              </div>
            </CardContent>
          </Card>
