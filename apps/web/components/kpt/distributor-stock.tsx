...
function AddStockDialog({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setOpen(false);
    setPartnerId(""); setInventoryItemId(""); setStockQty(""); setMinStockQty("10");
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Dispatch Stock to Distributor</DialogTitle>
        </DialogHeader>
...
export function DistributorStockPage() {
  ...
  const { data: partnersData, isLoading: loadingPartners } = usePartners({ type: "DISTRIBUTOR", limit: 50 });
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  {["Distributor", "SKU", "Product", "Category", "Qty at Partner", "Inventory Left", "Min Qty", "Unit Price", "Status", ""].map((h) => (
                ...
                {stock.length === 0 && (
                  <tr>
                    <td colSpan={10} className="px-6 py-8 text-center text-muted-foreground text-sm">
                      No stock dispatched to distributors yet.
                    </td>
                  </tr>
                )}
              </tbody>
