...
import { SegmentPayload, SegmentRuleType, SegmentRuleOperator } from "../lib/api/types"
...
export function SegmentFormModal({
  ...
  const updateRule = (ruleId: string, updater: Partial<RuleState>) => {
    setValues((prev) => ({
      ...prev,
      rules: prev.rules.map((rule) => (rule.id === ruleId ? { ...rule, ...updater } : rule)),
    }))
  }
  ...
  const removeRule = (ruleId: string) => {
    setValues((prev) => ({
      ...prev,
      rules: prev.rules.length > 1 ? prev.rules.filter((rule) => rule.id !== ruleId) : prev.rules,
    }))
  }
  ...
  const DialogContentAny = DialogContent as React.ComponentType<React.ComponentProps<typeof DialogContent>>
  const DialogHeaderAny = DialogHeader as React.ComponentType<React.ComponentProps<typeof DialogHeader>>
  const DialogTitleAny = DialogTitle as React.ComponentType<React.ComponentProps<typeof DialogTitle>>
  const DialogFooterAny = DialogFooter as React.ComponentType<React.ComponentProps<typeof DialogFooter>>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContentAny className="max-w-3xl">
        ...
        <div className="space-y-6">
          <div className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  setValues((prev) => ({ ...prev, logicOperator: val as "AND" | "OR" }))
                }
              >
                <SelectTrigger>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <Label>Rules</Label>
                <p className="text-sm text-muted-foreground">Add keyword or location filters</p>
              </div>
                ...
                <Card key={rule.id} className="p-4 space-y-4">
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="w-32">
                        ...
                        <Select
                          value={rule.operator}
                          onValueChange={(val) =>
                            updateRule(rule.id, { operator: val as SegmentRuleOperator })
                          }
                        >
                          <SelectTrigger>
                            <SelectValue />
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="NOT_IN">Does not match</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                    </div>
                  ...
                  {rule.ruleType === "KEYWORD" ? (
                    <KeywordSelect
                      ...
                      onSelectionChange={(ids) => updateRule(rule.id, { keywordIds: ids })}
                      label="Keywords"
                    />
                  ) : (
                    <div className="grid gap-2">
        ...
        <DialogFooterAny className="mt-6">
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={saving || isSubmitting}>
            Cancel
          </Button>
