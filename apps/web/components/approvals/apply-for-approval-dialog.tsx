...
export function ApplyForApprovalDialog({
  ...
  const { data: approvers = [], isLoading: approversLoading, isError: approversError } = useApprovers({ enabled: open })
  ...
  const handleSend = () => {
    ...
    submitMutation.mutate(
      { requestedToId: Number(assigneeId), comment: comment.trim() || undefined },
      {
        onSuccess: () => {
        ...
        onError: (err: any) => {
          ...
          if (code === "APPROVAL_EXISTS") {
            setErrorMessage("A pending approval already exists for this quote.")
          } else if (msg) {
            setErrorMessage(msg)
          } else {
            setErrorMessage("Failed to submit for approval.")
          }
          toast.error(code === "APPROVAL_EXISTS" ? "A pending approval already exists for this quote." : msg ?? "Failed to submit for approval.")
        },
      }
    )
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select an admin to review <span className="font-semibold">{quoteNumber}</span>.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            ...
            {approversError && (
              <p className="text-sm text-destructive">Failed to load approvers. You may not have permission to view the list.</p>
            )}
            {!approversError && selectableApprovers.length === 0 && !approversLoading && (
              <p className="text-sm text-muted-foreground">No other admins or system admins available to assign.</p>
            )}
          </div>
          <div className="space-y-2">
