...
export const AccountEditModal: React.FC<AccountEditModalProps> = ({ open, onOpenChange, initialValues, isSaving = false, onSave }) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContentAny>
        <DialogHeaderAny className="text-center">
          <DialogTitleAny className="text-center">Edit Account</DialogTitleAny>
        </DialogHeaderAny>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              <Label>Website</Label>
              <Input value={values.website || ""} onChange={(e) => setValues(v => ({ ...v, website: e.target.value }))} />
            </div>
            <div className="flex flex-col gap-2">
              <Label>Email</Label>
        ...
        <DialogFooterAny>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={saving || isSaving}>Cancel</Button>
          <Button onClick={handleSave} disabled={saving || isSaving}>{saving || isSaving ? "Saving..." : "Save"}</Button>
        </DialogFooterAny>
      </DialogContentAny> )})()}
    </Dialog>
  )
