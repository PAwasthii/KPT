...
function normalizeQuote(quote: Quote): Quote {
  ...
  return {
    ...quote,
    lineItemCount: Number.isFinite(quote.lineItemCount) ? quote.lineItemCount : Math.floor(Math.random() * 5) + 1,
...
function loadInitial(): Quote[] {
  ...
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify(DEFAULT_DUMMY_QUOTES))
...
export function useQuotesStore() {
  ...
  const byOpportunityId = React.useCallback(
    (opportunityId: string) => quotes.filter((q) => q.opportunityId === opportunityId),
    [quotes]
  )
