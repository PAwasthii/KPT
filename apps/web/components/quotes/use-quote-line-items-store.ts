...
function loadInitial(): QuoteLineItem[] {
  ...
  window.localStorage.setItem(STORAGE_KEY, JSON.stringify(DEFAULT_DUMMY_QUOTE_LINE_ITEMS))
...
export function useQuoteLineItemsStore() {
  ...
  const updateLineItem = React.useCallback((lineItemId: string, patch: Partial<QuoteLineItem>) => {
    setLineItems((prev) => {
