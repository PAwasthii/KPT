        ...
        {isLoading ? (
          ...
          <>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLES[inv.status] || 'bg-gray-100 text-gray-600'}`}>{inv.status.replace('_', ' ')}</span>
            </div>
...
export function InvoicesPage() {
  ...
  const { data: resp, isLoading, error } = useInvoices({ page, limit: 10, search: search || undefined, status: status || undefined });
  ...
  return (
    <div className="p-6 space-y-4 animate-kpt-fade-up">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {pagination && <span className="text-xs text-muted-foreground ml-auto">{pagination.total} invoices</span>}
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/30">
                    ...
                    <th className="text-right px-4 py-3 font-semibold text-xs uppercase tracking-wide text-muted-foreground">Balance</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {invoices.map((inv: any) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-amber-700">{f(Number(inv.totalAmount) - Number(inv.paidAmount))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
