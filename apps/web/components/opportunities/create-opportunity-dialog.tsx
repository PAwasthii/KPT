...
export function CreateOpportunityDialog({
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setIsSearching(true)
      try {
        const results = await accountService.searchAccounts(accountSearch.trim())
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2 md:col-span-2">
            ...
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter opportunity name" />
            {showErrors && !name.trim() && (
              <p className="text-xs text-red-500">Opportunity name is required</p>
            )}
          </div>
          ...
          <div className="space-y-2 relative" ref={accountSearchRef}>
            ...
            <Input
              ...
              onFocus={() => {
                if (accountResults.length > 0 && !accountId) setShowAccountResults(true)
              }}
            ...
            {showAccountResults && accountResults.length > 0 && (
              <div className="absolute z-50 top-full left-0 right-0 mt-1 max-h-48 overflow-y-auto rounded-md border border-gray-200 bg-white shadow-lg">
                {accountResults.map((account) => (
                  <button
                    key={account.id}
                    type="button"
                    className="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer transition-colors"
            ...
            {showAccountResults && accountResults.length === 0 && accountSearch.trim().length >= 2 && !isSearching && (
          ...
          <div className="space-y-2">
            <Label>Close Date <span className="text-red-500">*</span></Label>
