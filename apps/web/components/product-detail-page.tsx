...
import { Edit, Plus, Tag, Hash, CheckCircle2, Package, LayoutGrid, Calendar, Clock, FileText } from "lucide-react";
...
export function ProductDetailPage({
  ...
  const { data: pricebookEntriesData, isLoading: pricebookEntriesLoading } = usePricebookEntries({ productId });
  const { symbol: currencySymbol } = useCurrency();

  const [showAddPriceBookEntryModal, setShowAddPriceBookEntryModal] = React.useState(false);
  const [editingEntry, setEditingEntry] = React.useState<PriceBookEntry | null>(null);

  if (productLoading) return <HandleProductLoading />;
  if (productError || !product) return <HandleProductLoadError onBack={onBack} />;
  ...
  const pricebookEntryColumns: TableColumn<PriceBookEntry>[] = [
    ...
    },
    {
      ...
      label: 'Actions',
      render: (_, item) => (
        <Button variant="ghost" size="sm" onClick={() => handleEditPriceBookEntry(item)}>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="space-y-6 mt-6">
        <DetailCard title="Product Details" className="bg-white border-gray-200">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Product Name</p>
                <p className="text-sm font-medium text-gray-700">{productDetails.name}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Product Code</p>
                <p className="text-sm font-medium text-gray-700">{productDetails.code}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md ${productDetails.active ? "bg-green-50" : "bg-red-50"}`}>
                <CheckCircle2 className={`h-3.5 w-3.5 ${productDetails.active ? "text-green-500" : "text-red-500"}`} />
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Status</p>
                <p className={`text-sm font-medium ${productDetails.active ? "text-green-600" : "text-red-600"}`}>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Product Type</p>
                <p className="text-sm font-medium text-gray-700">{productDetails.component ? "Component" : "Finished Product"}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Category</p>
                <p className="text-sm font-medium text-gray-700">{productDetails.category?.name ?? "—"}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-blue-50">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-700">{new Date(productDetails.createdAt).toLocaleDateString("en-GB")}</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-700">{new Date(productDetails.updatedAt).toLocaleDateString("en-GB")}</p>
              </div>
            </div>
          </div>
          {productDetails.description && (
            <div className="mt-4 pt-4 border-t border-gray-100 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-700">{productDetails.description}</p>
              </div>
            </div>
          )}
        </DetailCard>
...
function HandleProductLoadError({ onBack }: { onBack: (() => void) | undefined }) {
  return (
