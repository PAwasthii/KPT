...
export function LeadsGeneratedCard() {
  ...
  const chartData = leadsData ? {
    ...
    datasets: (leadsData.datasets || []).map((dataset: any) => ({
      ...dataset,
      borderColor: '#009D9A',
      backgroundColor: 'rgba(0, 157, 154, 0.12)',
      pointBackgroundColor: '#009D9A',
...
export function ConversionRateCard() {
  ...
  const chartData = conversionData ? {
    ...conversionData,
    datasets: conversionData.datasets?.map((dataset: any) => ({
      ...dataset,
      backgroundColor: BRAND_CHART_COLORS.slice(0, dataset.data?.length || 0),
      borderColor: '#F7FAFC',
      borderWidth: 2,
    })) || []
  } : { labels: [], datasets: [] };
...
export function LeadSourcesCard() {
  ...
  const chartData = sourcesData ? {
    ...sourcesData,
    datasets: sourcesData.datasets?.map((dataset: any) => ({
      ...dataset,
      backgroundColor: BRAND_CHART_COLORS.slice(0, dataset.data?.length || 0),
      borderColor: '#F7FAFC',
      borderWidth: 2,
