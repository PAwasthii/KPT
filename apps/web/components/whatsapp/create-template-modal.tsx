...
import { X, Plus, Trash2, Bold, Italic, Strikethrough, Code, Image, Video, FileText, MapPin, AlertCircle } from "lucide-react";
...
const ALLOW_UTILITY_TEMPLATES = process.env.NEXT_PUBLIC_ALLOW_UTILITY_TEMPLATES === "true";
...
export function CreateTemplateModal({
  ...
  const handleCategoryChange = (newCategory: "MARKETING" | "UTILITY" | "AUTHENTICATION") => {
    if (newCategory === "AUTHENTICATION" && !ALLOW_AUTH_TEMPLATES) {
      toast.error("Authentication templates are disabled. Please choose Marketing or Utility.");
      return;
    }
  ...
  const [codeExpirationMinutes, setCodeExpirationMinutes] = useState<number | undefined>(undefined);
  ...
  const insertFormatting = (format: string) => {
    ...
    const updatedText = bodyText.substring(0, start) + newText + bodyText.substring(end);
  ...
  const handleMediaUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes[headerFormat]?.includes(file.type)) {
      toast.error(`Invalid file type. Allowed: ${validTypes[headerFormat]?.join(", ")}`);
      return;
    }
    ...
    if (buttons.length > 0) {
      components.push({
        ...
        buttons: buttons.map(btn => {
          ...
          const sanitizedExample = btn.example?.map((value) => value.trim()).filter(Boolean);
  ...
  const handleCreateTemplate = async (e: React.FormEvent) => {
    ...
    if (category !== "AUTHENTICATION" && bodyVariableCount > 0) {
      ...
      if (missingIndex !== -1) {
        const errorState = {
          ...
          details: [`Please provide a sample value for {{${missingIndex + 1}}}`],
        };
        setFormError(errorState);
        toast.error(errorState.title);
        return;
    ...
    if (["IMAGE", "VIDEO", "DOCUMENT"].includes(headerFormat) && !headerMediaHandle) {
      const errorState = { title: "Please upload a sample media file for template approval" };
      setFormError(errorState);
      toast.error(errorState.title);
      return;
    }
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-xl w-full max-w-6xl m-4 max-h-[90vh] overflow-hidden flex flex-col"
        onClick={(e) => {
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Form Section */}
          <form onSubmit={handleCreateTemplate} className="flex-1 p-6 space-y-6 overflow-y-auto">
            {formError && (
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  >
                    ...
                    {ALLOW_UTILITY_TEMPLATES && <option value="UTILITY">Utility</option>}
                    {ALLOW_AUTH_TEMPLATES && <option value="AUTHENTICATION">Authentication</option>}
                  </select>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    How long Meta retries sending if undelivered
                    (Default: 30 Days)
                  </p>
                </div>
              </div>
            ...
            <div className="border rounded p-4 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (WHATSAPP_TEXT_ONLY_MVP && value !== "NONE" && value !== "TEXT") {
                      toast.error("Only text headers are supported in MVP mode. Media and location headers will be available when MVP mode is disabled.");
                      return;
                    }
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <input
                      ...
                      maxLength={60}
                    />
                    <Button type="button" variant="outline" size="sm" onClick={addHeaderVariable}>
              ...
              {["IMAGE", "VIDEO", "DOCUMENT"].includes(headerFormat) && (
                <div className="space-y-3">
                  <input
                    ...
                    className="hidden"
                    accept={
                      headerFormat === "IMAGE" ? "image/jpeg,image/png,image/webp" :
                  ...
                  <div className={`p-3 rounded ${
                    headerFormat === "IMAGE" ? "bg-blue-50" :
                    headerFormat === "VIDEO" ? "bg-purple-50" : "bg-orange-50"
                  }`}>
                    <div className="flex items-center gap-2 mb-2">
                      {headerFormat === "IMAGE" && <Image className="h-5 w-5 text-blue-500" />}
                      {headerFormat === "VIDEO" && <Video className="h-5 w-5 text-purple-500" />}
                      {headerFormat === "DOCUMENT" && <FileText className="h-5 w-5 text-orange-500" />}
                      <span className="text-sm font-medium">
                        {headerFormat.charAt(0) + headerFormat.slice(1).toLowerCase()} Header
                      </span>
                    </div>
                    <p className="text-xs text-gray-600 mb-3">
                      Upload a sample {headerFormat.toLowerCase()} for Meta to review during approval.
                      The actual {headerFormat.toLowerCase()} will be provided when sending messages.
                    </p>

                    {headerMediaHandle ? (
                      <div className="flex items-center justify-between bg-white p-2 rounded border">
                        <span className="text-sm text-green-600 truncate">{headerMediaFileName}</span>
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
                      ...
                      <Button
                        ...
                        className="w-full"
                      >
                        {uploadingMedia ? "Uploading..." : `Upload Sample ${headerFormat.charAt(0) + headerFormat.slice(1).toLowerCase()}`}
                      </Button>
                    )}
                  </div>
                </div>
              ...
              {headerFormat === "LOCATION" && (
                <div className="flex items-center gap-2 text-sm text-gray-600 bg-green-50 p-3 rounded">
                  ...
                  <span>Location header - coordinates will be provided when sending messages</span>
                </div>
              )}
            </div>
            )}
            ...
            {category === "AUTHENTICATION" ? (
              <div className="border rounded p-4 space-y-3">
                ...
                <p className="text-sm text-gray-600">
                  WhatsApp automatically formats the OTP message body. Configure additional requirements below.
                </p>
                <div>
                  <label className="block text-sm font-medium mb-2">Code Expiration (minutes)</label>
                  <input
                    type="number"
                    className="w-full border rounded px-3 py-2"
                    ...
                    onChange={(e) => setCodeExpirationMinutes(e.target.value ? Number(e.target.value) : undefined)}
                  />
                  <p className="text-xs text-gray-500 mt-1">Optional expiry hint shown to customers.</p>
                </div>
                <label className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    checked={securityRecommendation}
                    onChange={(e) => setSecurityRecommendation(e.target.checked)}
                  />
              ...
              <div className="border rounded p-4 space-y-3">
                ...
                <div className="flex gap-1 mb-2">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={() => insertFormatting("strike")} title="Strikethrough">
                    <Strikethrough className="h-4 w-4" />
                  </Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => insertFormatting("code")} title="Code">
                    <Code className="h-4 w-4" />
                  </Button>
                  <Button type="button" variant="outline" size="sm" onClick={() => insertFormatting("variable")}>
                    <Plus className="h-4 w-4 mr-1" />
                    Variable
                  </Button>
                </div>
                ...
                <p className="text-xs text-gray-500">
                  Use {"{{1}}"}, {"{{2}}"} for variables. Format: *bold*, _italic_, ~strike~, ```code```
                </p>
                {bodyVariableCount > 0 && (
                  <div className="space-y-2 bg-gray-50 p-3 rounded">
                    <p className="text-xs font-medium text-gray-700">Sample values for variables:</p>
            ...
            {category === "AUTHENTICATION" ? (
              <div className="border rounded p-4 space-y-3">
                ...
                <p className="text-sm text-gray-600">
                  The Copy Code button is included automatically for authentication templates.
                </p>
              </div>
            ) : (
              <div className="border rounded p-4 space-y-3">
...
function buildTemplateFormError(error: any): TemplateFormError {
  const defaultTitle = typeof error?.message === "string" && error.message.trim().length
    ? error.message.trim()
  ...
  if (!details.size && error?.status === 400) {
    details.add("Double-check template fields for Meta/MSG91 validation requirements and try again.");
  }
...
function extractMsg91ErrorDetails(rawMessage: string): string[] {
  ...
  if (lowerMessage.includes("phone number")) {
    details.add("Call buttons must use a valid E.164 phone number (e.g. +911234567890) with digits only.");
  }
  if (lowerMessage.includes("variables can't be at the start or end")) {
    details.add("Add fixed text before and after each {{variable}} in header and body copy.");
  }
