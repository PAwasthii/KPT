...
import { Card, Button, Input, Textarea, Label, Badge } from "@repo/ui"
...
import { whatsappService, segmentService } from "@/lib/api/services"
import { useSegments } from "@/hooks/useSegments"
import type { MessageTemplate } from "@/lib/api/types"
import { ChevronLeft, ChevronRight, Search, ChevronsUpDown, Check, Database, UploadCloud, FileSpreadsheet, X } from "lucide-react"
...
const ALLOW_AUTH_TEMPLATES = process.env.NEXT_PUBLIC_ALLOW_AUTH_TEMPLATES === "true"
const WHATSAPP_TEXT_ONLY_MVP = process.env.NEXT_PUBLIC_WHATSAPP_TEXT_ONLY_MVP === "true"

// Available database fields for dynamic values
const AVAILABLE_FIELDS = [
  { value: "{{name}}", label: "Name", description: "Contact name" },
  { value: "{{firstName}}", label: "First Name", description: "Lead first name" },
  { value: "{{lastName}}", label: "Last Name", description: "Lead last name" },
  { value: "{{email}}", label: "Email", description: "Email address" },
  { value: "{{phone}}", label: "Phone", description: "Phone number" },
  { value: "{{countryCode}}", label: "Country Code", description: "Country code" },
  ...
  { value: "{{companyName}}", label: "Company Name", description: "Lead company name" },
]
...
function FieldInputWithDropdown({ fieldKey, label, type, placeholder, value, onChange, csvColumns }: FieldInputWithDropdownProps) {
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              className="px-3"
              type="button"
              title={hasCsvColumns ? "Insert CSV column or database field" : "Insert database field"}
            >
...
function MediaUploadField({ fieldKey, label, mediaType, uploading, handle, fileName, disabled, onUpload, onUrlChange, urlValue }: MediaUploadFieldProps) {
  const inputRef = useRef<HTMLInputElement>(null)
      ...
      {mode === 'url' ? (
        <div className="space-y-2">
          <Input
            ...
            disabled={disabled}
          />
          {urlValue && (urlValue.startsWith('http://') || urlValue.startsWith('https://')) && (
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {uploading ? "Uploading to S3..." : urlValue ? "Replace File" : "Upload File"}
            </Button>
            {urlValue && (
              <span className="text-sm text-green-600 truncate max-w-[220px]" title={urlValue}>
                ✓ Uploaded to S3
              </span>
            )}
          </div>
          <p className="text-xs text-muted-foreground">
            Upload file to S3 (max 10MB). Files are stored in whatsapp-campaign folder.
          </p>
          <input
            ref={inputRef}
            type="file"
...
export function CreateCampaignModal({ open, onOpenChange, onSuccess, editCampaignId }: CreateCampaignModalProps) {
  ...
  const [mediaParams, setMediaParams] = useState<Record<string, { handle: string; mediaType: string; fileName?: string }>>({})
  const [mediaUploadStatus, setMediaUploadStatus] = useState<Record<string, boolean>>({})
  ...
  const [segmentPreview, setSegmentPreview] = useState<{ total: number; sample: number }>({ total: 0, sample: 0 })
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return base.filter((t: MessageTemplate) => 
  ...
  const templateInsights = useMemo(() => buildTemplateInsights(selectedTemplate), [selectedTemplate])
  
  // Get available languages from selected template
  const availableLanguages = useMemo(() => {
    if (!selectedTemplate) return []
    
    if (selectedTemplate.languages && Array.isArray(selectedTemplate.languages) && selectedTemplate.languages.length > 0) {
      return selectedTemplate.languages
    }
    
    // Fallback to single language
    if (selectedTemplate.language) {
      return [{ code: selectedTemplate.language, status: selectedTemplate.status || "APPROVED" }]
    }
        ...
        if (config.messageParams && typeof config.messageParams === 'object') {
          ...
          Object.entries(config.messageParams).forEach(([key, value]) => {
            if (typeof value === 'string') {
              ...
              parsedParams[key] = String((value as Record<string, unknown>).value)
            }
          })
    ...
    const loadTemplates = async () => {
      try {
        ...
        if (WHATSAPP_TEXT_ONLY_MVP) {
          filtered = filtered.filter((tpl: MessageTemplate) => isTextOnlyTemplate(tpl))
        }
        setTemplates(filtered)
        setTemplateName((prev) => {
          if (!prev) return prev
          const exists = filtered.some((tpl: MessageTemplate) => tpl.name === prev)
          if (!exists) {
            setSelectedLanguage("")
            return ""
          }
  ...
  const handleCsvUpload = async (file: File) => {
    ...
    const loadingToast = file.size > 1024 * 1024 ? toast.loading("Parsing file...") : null
    ...
    try {
      ...
      reader.onload = (e) => {
        try {
          ...
          const contacts = jsonData.slice(1)
            .map(row => {
              ...
              headers.forEach((header, index) => {
                contact[header] = row[index] != null ? String(row[index]).trim() : ''
              })
              return contact
            })
          ...
          const phoneCol = headers.find(col =>
          ...
          if (loadingToast) toast.dismiss(loadingToast)
          toast.success(`Successfully loaded ${contacts.length} contact${contacts.length > 1 ? 's' : ''} from ${fileType} file`)
          setError(null)
          setCsvLoading(false)
        } catch (error: any) {
          console.error('File parsing error:', error)
          if (loadingToast) toast.dismiss(loadingToast)
          toast.error(`Failed to parse file: ${error.message || 'Invalid file format'}`)
          setCsvLoading(false)
          setError(`File parsing failed: ${error.message || 'Invalid file format'}`)
        }
      }
  ...
  const handleMediaUpload = async (fieldKey: string, file: File, mediaType: string) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      mediaFieldKeysRef.current.forEach((key) => {
        ...
        const mediaTypeFromTemplate = mediaFieldTypesRef.current[key] || 'IMAGE'

        // Check if user entered a URL
        if (urlValue && urlValue.trim() && (urlValue.startsWith('http://') || urlValue.startsWith('https://'))) {
        ...
        // Check if user uploaded media (fallback - not recommended for campaigns)
          ...
          throw new Error(`Please provide a URL for ${key}. Media must be publicly accessible.`)
        }
      })
      ...
      const payload = {
        ...
        scheduledAt: campaignAction === "schedule" ? new Date(`${scheduleDate}T${scheduleTime}`).toISOString() : undefined,
      ...
      if (campaignAction === "send") {
        ...
        await whatsappService.scheduleCampaign(campaignId, new Date(`${scheduleDate}T${scheduleTime}`).toISOString())
      }
      ...
      setError(err?.message || (isEditMode ? "Failed to update campaign" : "Failed to create campaign"))
    } finally {
  ...
  const renderStep = () => {
    switch (currentStep) {
              ...
              <Card className="p-4 bg-muted/40 border-dashed space-y-2">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedTemplate.status && <Badge variant="outline">{selectedTemplate.status}</Badge>}
                    {selectedTemplate.category && <Badge variant="secondary">{selectedTemplate.category}</Badge>}
                  </div>
                </div>
                {WHATSAPP_TEXT_ONLY_MVP && (
                  <p className="text-xs text-muted-foreground pt-1">
                    Only text-only WhatsApp templates are available in this MVP. Media templates will be supported later.
                  </p>
                )}
              </Card>
            )}
            ...
            {toAudience === "segment" && (
              <div className="grid gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/campaigns/segments" className="text-sm text-primary hover:underline">
                    Manage segments
                  </Link>
                </div>
                <Select
                ...
                <div className="flex items-center gap-2">
                  ...
                  {segmentPreview.total > 0 && (
                    <span className="text-sm text-muted-foreground">
                      {segmentPreview.total} contacts match (showing first {segmentPreview.sample})
                    </span>
                  )}
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 space-y-1">
                        <p className="text-sm font-medium">Upload Contacts File</p>
                        <p className="text-xs text-muted-foreground">
                          Upload a CSV or Excel file with your contacts. The file should contain phone numbers with country code (without + sign).
                        </p>
                      </div>
                    </div>
                    ...
                    {!csvFile ? (
                      <div>
                        ...
                        >
                          <UploadCloud className="h-4 w-4 mr-2" />
                          {csvLoading ? "Parsing file..." : "Choose CSV/Excel File"}
                        </Button>
                        <input
                          ref={fileInputRef}
                          type="file"
                        ...
                        {csvLoading && (
                          <p className="text-xs text-blue-600 mt-2 flex items-center gap-1">
                            <span className="animate-spin">⏳</span> Processing file, please wait...
                          </p>
                        )}
                      </div>
                    ) : (
                      <div className="space-y-3">
                        ...
                        <div className="grid gap-2">
                          <Label htmlFor="phoneColumn">Phone Number Column</Label>
                          <Select
                    ...
                    <div className="border-t pt-3 space-y-1">
                      ...
                      <ul className="text-xs text-muted-foreground space-y-1 list-disc list-inside">
                        ...
                        <li>Phone numbers must include country code without + sign (e.g., 919876543210)</li>
      ...
      case 6:
        ...
        const paramFields = templateInsights.hints.map((hint) => {
          ...
          const headerMediaMatch = hint.match(/Header uses (\w+) media \(configure (\w+)\)/)
          ...
          if (bodyMatch && bodyMatch[1]) {
            ...
            return vars.map((v) => {
              ...
              return { key: `body_${num}`, label: `Body Variable ${num}`, type: 'text', placeholder: `Enter value for {{${num}}}` }
            })
          }
          if (headerMatch && headerMatch[1]) {
            ...
            return vars.map((v) => {
              const num = v.match(/\d+/)?.[0] || '1'
              return { key: `header_${num}`, label: `Header Variable ${num}`, type: 'text', placeholder: `Enter value for {{${num}}}` }
            })
          }
          if (headerMediaMatch) {
            const [, mediaType = 'image', key = 'header_1'] = headerMediaMatch
            return [{
              key,
          ...
          if (buttonMatch) {
            ...
            if (btnType === 'URL') {
              return [{ key: `button_${btnNum}`, label: `Button ${btnNum} URL`, type: 'url', placeholder: 'https://example.com/page' }]
            }
            if (btnType === 'Copy') {
              return [{ key: `button_${btnNum}`, label: `Button ${btnNum} Coupon Code`, type: 'text', placeholder: 'DISCOUNT20' }]
            }
          }
          return []
        }).flat()
        ...
        // Deduplicate fields
        const uniqueFields = paramFields.filter((field, index, self) =>
          field && field.key && index === self.findIndex((f) => f?.key === field.key)
        )
        ...
        const mediaFieldsMap = new Map(
          uniqueFields
            ...
            .map((field) => [field!.key as string, (field as any).mediaType || 'IMAGE'])
        )
        ...
        const buildPreviewComponents = () => {
          ...
          const components = extractComponentsArray(
            (selectedTemplate as any)?.componentsJson ?? (selectedTemplate as any)?.components ?? null
          )
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Fill in the values for the template placeholders. Use the database icon to insert dynamic fields.
                </p>
              </div>

              {uniqueFields.length > 0 ? (
                <div className="space-y-4">
                  {uniqueFields.map((field) => field && field.key && (
                    field.type === 'media' ? (
                      <MediaUploadField
                        key={field.key}
                        fieldKey={field.key}
                        ...
                        handle={mediaParams[field.key]?.handle}
                        fileName={mediaParams[field.key]?.fileName}
                        disabled={!accountId}
                        onUpload={(file) => handleMediaUpload(field.key!, file, (field as any).mediaType || 'IMAGE')}
                        onUrlChange={(url) => setParamValues(prev => ({ ...prev, [field.key!]: url }))}
                      ...
                      <FieldInputWithDropdown
                        ...
                        csvColumns={toAudience === "upload" ? csvColumns : undefined}
                      />
                    )
                  ))}
                </div>
                ...
                <Card className="p-4 bg-muted/40 border-dashed space-y-4">
                  ...
                  <div className="space-y-3">
                    {Object.keys(JSON.parse(templateInsights.sampleParamsText || '{}')).map((key) => (
                      <FieldInputWithDropdown
                        key={key}
                        fieldKey={key}
                ...
                <Card className="p-4 bg-muted/40 border-dashed">
                  <p className="text-sm text-muted-foreground text-center">
                    This template has no variable placeholders. You can proceed to the next step.
                  </p>
                </Card>
              )}
            </div>
            ...
            <div className="grid gap-2">
              ...
              <div className="grid gap-3">
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`w-4 h-4 mt-0.5 rounded-full border-2 flex items-center justify-center ${
                      campaignAction === "send" ? "border-primary" : "border-muted-foreground"
                    }`}>
                      {campaignAction === "send" && <div className="w-2 h-2 rounded-full bg-primary" />}
                    </div>
                    <div>
                      <h4 className="font-medium">Send Now</h4>
                      <p className="text-sm text-muted-foreground">
                        Create and send the campaign immediately to all recipients
                      </p>
                    </div>
                  </div>
                </Card>
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`w-4 h-4 mt-0.5 rounded-full border-2 flex items-center justify-center ${
                      campaignAction === "schedule" ? "border-primary" : "border-muted-foreground"
                    }`}>
                      {campaignAction === "schedule" && <div className="w-2 h-2 rounded-full bg-primary" />}
                    </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`w-4 h-4 mt-0.5 rounded-full border-2 flex items-center justify-center ${
                      campaignAction === "draft" ? "border-primary" : "border-muted-foreground"
                    }`}>
                      {campaignAction === "draft" && <div className="w-2 h-2 rounded-full bg-primary" />}
                    </div>
                    <div>
                      <h4 className="font-medium">Save as Draft</h4>
                      <p className="text-sm text-muted-foreground">
                        Save the campaign without sending. You can send it later from the campaigns list.
                      </p>
                    </div>
                  </div>
                </Card>
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      >
        <DialogHeader>
          <DialogTitle>{isEditMode ? "Edit WhatsApp Campaign" : "Create WhatsApp Campaign"}</DialogTitle>
...
function buildTemplateInsights(template?: MessageTemplate | any): TemplateInsights {
  if (!template) {
    return { sampleParamsText: "", rawJson: "", hints: [] }
  }

  const raw = (template as any)?.componentsJson ?? (template as any)?.components ?? null
  ...
  return {
    sampleParamsText: Object.keys(sample).length ? JSON.stringify(sample, null, 2) : "",
...
function processComponent(component: any, sample: Record<string, any>, hints: string[]) {
  ...
  switch (type) {
    ...
    default:
      if (Array.isArray((component as any).cards)) {
        hints.push("Template contains carousel cards. Provide card_X components such as card_0_header_1.")
      }
  }
}

function handleHeaderComponent(component: any, sample: Record<string, any>, hints: string[]) {
  const format = String(component.format || component.format_type || "").toUpperCase()
  const headerText = typeof component.text === "string" ? component.text : undefined
  const exampleText = getExampleText(component, "header_text")

  if (format === "IMAGE" || format === "VIDEO" || format === "DOCUMENT") {
    sample.header_1 = buildMediaExample(format)
    hints.push(`Header uses ${format.toLowerCase()} media (configure header_1)`)
    return
  }
  ...
  if (format === "LOCATION") {
    ...
    hints.push("Header uses location (provide header_1_latitude / longitude / name / address)")
    return
  }
...
function handleBodyComponent(component: any, sample: Record<string, any>, hints: string[]) {
  const bodyText = typeof component.text === "string" ? component.text : undefined
...
function handleButtonsComponent(component: any, sample: Record<string, any>, hints: string[]) {
  const buttons = getButtonList(component)
  if (!buttons.length) {
  ...
  buttons.forEach((button: any, index: number) => {
    ...
    const phoneValue = typeof button.phone_number === "string" ? button.phone_number : undefined
    const couponValue = typeof button.coupon_code === "string" ? button.coupon_code : undefined

    switch (type) {
      case "URL":
      case "VISIT_WEBSITE": {
        if (hasTemplatePlaceholders(urlValue)) {
          sample[key] = { type: "text", subtype: "url", value: "https://example.com" }
          hints.push(`Button ${index + 1}: URL – configure ${key}.value with a link`)
        }
        break
      }
      case "PHONE_NUMBER":
      case "CALL_PHONE_NUMBER": {
        if (hasTemplatePlaceholders(phoneValue)) {
          sample[key] = { type: "text", subtype: "phone_number", value: "+1234567890" }
          hints.push(`Button ${index + 1}: Phone call – configure ${key}.value with a phone number`)
        }
        break
      }
      case "COPY_CODE":
      case "COUPON_CODE": {
        if (hasTemplatePlaceholders(couponValue)) {
          sample[key] = { type: "coupon_code", coupon_code: "OFFER2025", subtype: "COPY_CODE" }
          hints.push(`Button ${index + 1}: Copy code – configure ${key}.coupon_code`)
        }
        break
      }
      case "CATALOG":
      ...
      case "MPM": {
        if (hasTemplatePlaceholders(button.value)) {
          ...
          hints.push(`Button ${index + 1}: Catalog/Action – configure ${key}.value with product data`)
        }
        break
      }
      default:
...
function assignTextVariables(
  ...
  hints.push(`${label} variables: ${placeholders.map((p) => `{{${p}}}`).join(", ")}`)
}
...
function extractPlaceholders(text?: string): number[] {
  ...
  const numbers = matches.map((match) => Number(match[1])).filter((num) => !Number.isNaN(num))
