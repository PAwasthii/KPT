...
export function formatAnalyticsTitle(eventType?: string) {
  ...
  if (eventType.startsWith("whatsapp.")) {
    ...
    return statusMap[status] || `WhatsApp ${status.charAt(0).toUpperCase() + status.slice(1)}`
  }

  const parts = eventType.split(".").filter(Boolean)
  const normalized = parts.join(" ")
  return normalized.replace(/[_-]/g, " ").replace(/\b\w/g, (char) => char.toUpperCase()) || "Activity"
}
...
export function formatAnalyticsDescription(event: AnalyticsEvent) {
  ...
  if (data && typeof data === "object") {
    ...
    const error = (data as any).errorMessage || (data as any).error || (data as any).reason
    ...
    if (event.eventType === "whatsapp.opt_out_removed") {
      ...
      const pieces = [
        ...
        previousCampaignName ? `Previous campaign: ${previousCampaignName}` : null,
      ].filter(Boolean)
      return pieces.join(" • ")
    }
    ...
    const address = (data as any).address || (data as any).phone || (data as any).mobile
    const pieces = [
      campaignName ? `Campaign: ${campaignName}` : null,
      status ? `Status: ${status}` : null,
