...
async function imageToBase64(imagePath: string): Promise<string | null> {
  try {
    // In browser context, we'll need to fetch the image
...
export async function generateCampaignPDF(
  ...
  const addStatRow = (label: string, value: string | number, y: number, indent: number = 0) => {
  ...
  const openRate = globalStats?.opensRate ?? (legacyStats as any)?.openPercentage ?? safeRate(opens, delivered || sent);
  const clickThroughRate = (legacyStats as any)?.clickPercentage ?? safeRate(clicks, delivered || sent);
  const clickToOpenRate = safeRate(clicks, opens);
  const unsubscribeRate = (legacyStats as any)?.unsubscriptionPercentage ?? safeRate(unsubscribes, delivered || sent);
  const spamComplaintRate = (legacyStats as any)?.spamPercentage ?? safeRate(spamReports, delivered || sent);
  ...
  const addStatRowInColumn = (label: string, value: string | number, y: number, x: number) => {
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
    doc.text(`${label}:`, x, y);
  ...
  doc.text('• Apple MPP opens included.', notesX, notesStartY + 5, { align: 'left' });
  doc.text('• Bot opens and clicks included.', notesX, notesStartY + 10, { align: 'left' });
  ...
  leftY = addStatRowInColumn('Delivery rate', `${deliveryRate.toFixed(2)}%`, leftY, leftColumnX);
  leftY = addStatRowInColumn('Soft bounces', `${softBounces} (${softBounces > 0 ? safeRate(softBounces, sent).toFixed(2) : 0}%)`, leftY, leftColumnX);
  leftY = addStatRowInColumn('Hard bounces', `${hardBounces} (${hardBounces > 0 ? safeRate(hardBounces, sent).toFixed(2) : 0}%)`, leftY, leftColumnX);
  leftY += 5;
  ...
  leftY = addStatRowInColumn('Open rate', `${openRate.toFixed(2)}%`, leftY, leftColumnX);
  leftY = addStatRowInColumn('Total opens', totalOpens, leftY, leftColumnX);
  leftY = addStatRowInColumn('Apple MPP opens', appleMPPOpens, leftY, leftColumnX);
  ...
  rightY = addStatRowInColumn('Click-through rate', `${clickThroughRate.toFixed(2)}%`, rightY, rightColumnX);
  rightY = addStatRowInColumn('Total clicks', totalClicks, rightY, rightColumnX);
  rightY = addStatRowInColumn('Click-to-open rate', `${clickToOpenRate.toFixed(2)}%`, rightY, rightColumnX);
  ...
  rightY += 2;
  rightY = addStatRowInColumn('Unsubscribe rate', `${unsubscribeRate.toFixed(2)}%`, rightY, rightColumnX);
  rightY = addStatRowInColumn('Spam complaints', spamReports, rightY, rightColumnX);
  ...
  doc.text(`Generated on ${exportedDate}`, pageWidth / 2, footerY, { align: 'center' });
