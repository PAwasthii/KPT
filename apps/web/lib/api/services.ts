...
export const userService = {
  ...
  importUsers: async (users: Array<{
    ...
    message: string;
    success: Array<{ row: number; email: string; firstName: string; lastName: string }>;
  ...
  importUsersFile: async (file: File): Promise<{
    message: string;
    success: Array<{ row: number; email: string; firstName: string; lastName: string }>;
    errors: Array<{ row: number; firstName: string; lastName: string; email: string; phone: string; role: string; region: string; error: string }>;
    report?: { filename: string; mimeType: string; base64: string };
  }> => {
...
export const whatsappService = {
  ...
  uploadTemplateMedia: async (data: {
    ...
    const response = await apiClient.post("/api/whatsapp/templates/upload-media", data);
    return response.data;
  },
  uploadCampaignMedia: async (data: {
    ...
    const response = await apiClient.post("/api/whatsapp/campaigns/upload-media", data);
    return response.data;
  },
  ...
  getCampaignConfig: async (id: number): Promise<{
    id: number;
    name: string;
    description?: string;
    accountId: number;
    ...
    const response = await apiClient.get(`/api/whatsapp/campaigns/${id}/config`);
    return response.data;
  },
  ...
  updateCampaign: async (id: number, payload: any): Promise<any> => {
    const response = await apiClient.put(`/api/whatsapp/campaigns/${id}`, payload);
    return response.data;
  },
  ...
  scheduleCampaign: async (id: number, scheduledAt: string): Promise<{ scheduled: boolean }> => {
    const response = await apiClient.post(`/api/whatsapp/campaigns/${id}/schedule`, { scheduledAt });
    return response.data;
  },
  // Data
...
export const subdealerService = {
  ...
  login: async (data: SubdealerLoginRequest): Promise<SubdealerAuthResponse> => {
...
export const productService = {
  ...
  createProduct: async (data: CreateProductInput): Promise<ApiResponse<Product>> => {
    ...
    if (data.component !== undefined) formData.append("component", data.component.toString());
...
export const pricebookService = {
  ...
  updatePriceBook: async (id: number, data: Partial<PriceBook>): Promise<PriceBook> => {
...
export const pricebookEntryService = {
  getAllPriceBookEntries: async (filters: { productId: number }): Promise<PaginatedApiResponse<PriceBookEntry>> => {
  ...
  getPriceBookEntriesByPriceBookId: async (
    filters: { priceBookId: number }
  ): Promise<{ data: PriceBookEntry[] }> => {
    const response = await apiClient.get(`/api/pricebook-entries/pricebook/${filters.priceBookId}`, {
      params: filters,
    });
    return response.data;
  },
  ...
  createPriceBookEntry: async (data: Partial<PriceBookEntry>): Promise<PriceBookEntry> => {
    const response = await apiClient.post("/api/pricebook-entries", data);
    return response.data;
  },

  updatePriceBookEntry: async (id: number, data: Partial<PriceBookEntry>): Promise<PriceBookEntry> => {
...
export const segmentService = {
  ...
  resolve: async (id: number, limit?: number): Promise<SegmentResolveResponse> => {
    const response = await apiClient.post(`/api/segments/${id}/resolve`, { limit });
    return response.data;
  },
};
...
export const quoteService = {
  ...
  getQuoteLineItems: async (
    ...
    const response = await apiClient.get(`/api/quotes/${quoteId}/line-items`, { params })
    return response.data
  },
  getQuoteOrders: async (quoteId: number): Promise<PaginatedApiResponse<QuoteOrderItem>> => {
  ...
  downloadPdf: async (quoteId: number, quoteNumber: string): Promise<void> => {
    const response = await apiClient.get(`/api/quotes/${quoteId}/pdf`, { responseType: 'blob' })
  ...
  setPrimary: async (quoteId: number): Promise<QuoteDetail> => {
    const response = await apiClient.patch(`/api/quotes/${quoteId}/set-primary`)
    return response.data
  },
...
export const orderService = {
  createOrder: async (data: {
    ...
    const token = typeof window !== 'undefined' ? localStorage.getItem('subdealer_token') : null;
...
export const invoiceService = {
  uploadInvoice: async (file: File, phone: string): Promise<ApiResponse<any>> => {
    const formData = new FormData();
    formData.append("file", file);
...
export const settingsService = {
  ...
  updateGlobalSetting: async (key: string, value: string): Promise<any> => {
    const response = await apiClient.put("/api/settings/global-settings", { key, value });
    return response.data;
  },
...
const getAakramanConfig = () => {
  const token = typeof window !== 'undefined' ? localStorage.getItem('aakraman_token') : null;
  return token ? { headers: { Authorization: `Bearer ${token}` } } : {};
};

export const aakramanService = {
  // OTP Methods
  sendSmsOtp: async (phone: string): Promise<{ success: boolean; message: string; expiresIn: number }> => {
    const response = await apiClient.post("/api/aakraman/send-otp/sms", { phone });
    return response.data;
  },

  sendEmailOtp: async (email: string): Promise<{ success: boolean; message: string; expiresIn: number }> => {
    const response = await apiClient.post("/api/aakraman/send-otp/email", { email });
    return response.data;
  },

  verifyOtp: async (data: { phone?: string; email?: string; otp: string }): Promise<{
    success: boolean;
    message: string;
    token: string;
    user: AakramanUser;
  ...
  getCurrentUser: async (): Promise<{ user: AakramanUser }> => {
    const response = await apiClient.get("/api/aakraman/me", getAakramanConfig());
    return response.data;
  },

  // Product Methods
  getProducts: async (params?: { search?: string; categoryId?: number }): Promise<{
  ...
  createOrder: async (data: {
    ...
    const response = await apiClient.post("/api/aakraman/orders", data, getAakramanConfig());
    return response.data;
  },

  getMyOrders: async (): Promise<{ orders: AakramanOrder[] }> => {
    const response = await apiClient.get("/api/aakraman/orders", getAakramanConfig());
    return response.data;
  },
  ...
  admin: {
    getAllOrders: async (params?: {
      ...
      const response = await apiClient.get("/api/aakraman/admin/orders", { params });
      return response.data;
    },
    ...
    updateOrder: async (id: number, data: Partial<{
      ...
      const response = await apiClient.put(`/api/aakraman/admin/orders/${id}`, data);
      return response.data;
    },

    getSalesUsers: async (): Promise<{ users: Array<{
      id: number;
      firstName: string | null;
      lastName: string | null;
      email: string;
    ...
    archiveOrder: async (id: number): Promise<{ success: boolean; message: string; order: AakramanOrder }> => {
      const response = await apiClient.post(`/api/aakraman/admin/orders/${id}/archive`);
      return response.data;
    },

    unarchiveOrder: async (id: number): Promise<{ success: boolean; message: string; order: AakramanOrder }> => {
      const response = await apiClient.post(`/api/aakraman/admin/orders/${id}/unarchive`);
      return response.data;
    },
  },
};
...
export const opportunityService = {
  ...
  getOpportunityById: async (id: number): Promise<{ data: OpportunityDetail }> => {
    const response = await apiClient.get(`/api/opportunities/${id}`);
    return response.data;
  },

  create: async (data: CreateOpportunityInput): Promise<{ data: OpportunityDetail }> => {
    const response = await apiClient.post("/api/opportunities", data);
    return response.data;
  },

  getOpportunityLineItems: async (opportunityId: number): Promise<{ data: OpportunityLineItem[] }> => {
    const response = await apiClient.get(`/api/opportunities/${opportunityId}/line-items`);
    return response.data;
  },
  ...
  getOpportunityQuotes: async (
    ...
    const response = await apiClient.get(`/api/opportunities/${opportunityId}/quotes`, { params });
    return response.data;
  },
  ...
  deleteOpportunityLineItem: async (opportunityId: number, lineItemId: number): Promise<void> => {
    await apiClient.delete(`/api/opportunities/${opportunityId}/line-items/${lineItemId}`);
  },
...
export const notificationService = {
  getAll: async (page = 1): Promise<{
    ...
    const response = await apiClient.get('/api/notifications', { params: { page } });
    return response.data;
  },
...
export const salesOrderService = {
  getAllOrders: async (params?: { page?: number; limit?: number }): Promise<{ data: SalesOrderListItem[]; pagination: any }> =>
    apiClient.get('/api/sales-orders', { params }).then(r => r.data),

  getOrderById: async (id: number): Promise<{ data: SalesOrderDetail }> =>
    apiClient.get(`/api/sales-orders/${id}/get-order-details`).then(r => r.data),
  ...
  downloadPdf: async (id: number, orderNumber: string): Promise<void> => {
    const response = await apiClient.get(`/api/sales-orders/${id}/pdf`, { responseType: 'blob' });
    const url = URL.createObjectURL(response.data);
    const a = document.createElement('a');
    a.href = url;
...
export const approvalService = {
  ...
  getApprovalById: async (id: number): Promise<{ data: ApprovalProcessApi }> => {
...
// ============================================
// KPT — Channel Partners & Incentives
// ============================================
export const kptPartnerService = {
  getAll: async (params?: {
    page?: number;
    limit?: number;
    type?: string;
    ...
    const response = await apiClient.get('/api/kpt/channel-partners', { params });
    return response.data;
  },

  getById: async (id: number) => {
    const response = await apiClient.get(`/api/kpt/channel-partners/${id}`);
    return response.data;
  },
  ...
  create: async (data: {
    ...
    const response = await apiClient.post('/api/kpt/channel-partners', data);
    return response.data;
  },

  update: async (id: number, data: Record<string, any>) => {
    const response = await apiClient.put(`/api/kpt/channel-partners/${id}`, data);
    return response.data;
  },
  ...
  getIncentives: async (partnerId: number) => {
    const response = await apiClient.get(`/api/kpt/channel-partners/${partnerId}/incentives`);
    return response.data;
  },
...
// ============================================
// KPT — Stock Visibility
// ============================================
export const kptStockService = {
  getAll: async (params?: {
    page?: number;
    limit?: number;
    partnerId?: number;
    ...
    const response = await apiClient.get('/api/kpt/stock', { params });
    return response.data;
  },

  getAlerts: async () => {
    const response = await apiClient.get('/api/kpt/stock/alerts');
    return response.data;
  },

  getByPartner: async (partnerId: number) => {
    const response = await apiClient.get(`/api/kpt/stock/partner/${partnerId}`);
    return response.data;
  },
  ...
  create: async (data: {
    ...
    const response = await apiClient.post('/api/kpt/stock', data);
    return response.data;
  },

  update: async (id: number, data: Record<string, any>) => {
    const response = await apiClient.put(`/api/kpt/stock/${id}`, data);
    return response.data;
  },

  delete: async (id: number) => {
    const response = await apiClient.delete(`/api/kpt/stock/${id}`);
    return response.data;
  },
};
...
// KPT — Incentive Slabs
// ============================================
export const kptIncentiveSlabService = {
  getAll: async (params?: { tier?: string; isActive?: boolean }) => {
    const response = await apiClient.get('/api/kpt/incentive-slabs', { params });
    return response.data;
  },
  ...
  create: async (data: {
    ...
    const response = await apiClient.post('/api/kpt/incentive-slabs', data);
    return response.data;
  },

  update: async (id: number, data: Record<string, any>) => {
    const response = await apiClient.put(`/api/kpt/incentive-slabs/${id}`, data);
    return response.data;
  },

  delete: async (id: number) => {
    const response = await apiClient.delete(`/api/kpt/incentive-slabs/${id}`);
    return response.data;
  },
};

// ============================================
// KPT — Performance & Analytics
// ============================================
export const kptPerformanceService = {
  getRevenueSummary: async () => {
    const response = await apiClient.get('/api/kpt/performance/revenue');
    return response.data;
  },

  getPartnerRankings: async () => {
    const response = await apiClient.get('/api/kpt/performance/rankings');
    return response.data;
  },

  getMonthlyTrends: async () => {
    const response = await apiClient.get('/api/kpt/performance/trends');
    return response.data;
  },
