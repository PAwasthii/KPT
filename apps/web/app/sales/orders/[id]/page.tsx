...
import {
  Download, Loader2, Hash, Calendar, FileText, TrendingUp,
  DollarSign, Tag, Package, Building2, User, Mail, Phone,
  Clock, CheckCircle2, MapPin, Truck, StickyNote, CreditCard,
} from "lucide-react"
import {
  Badge,
  Button,
...
function OrderDetailContent() {
  ...
  const handleDownloadPdf = React.useCallback(async (orderId: number, orderNumber: string) => {
  ...
  const statusBadgeClass =
    STATUS_BADGE_CLASSES[order.status] ?? "bg-gray-100 text-gray-800 border-gray-200"

  const billingCityLine = [order.billingCity, order.billingState, order.billingPostalCode]
    .filter(Boolean)
    .join(", ")

  const shippingCityLine = [order.shippingCity, order.shippingState, order.shippingPostalCode]
    .filter(Boolean)
    .join(", ")
          ...
          <TabsContent value="details">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                <DetailCard title="Order Information" className="bg-white border-gray-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Order Number</p>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Order Date</p>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-700">{order.quote?.quoteNumber ?? "—"}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3 sm:col-span-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-700">{order.quote?.opportunity?.name ?? "—"}</p>
                      </div>
                    </div>
                  </div>
                </DetailCard>
                  ...
                  <div className="rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 p-4 text-white mb-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-6 text-sm opacity-90">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{formatPercent(order.discountPercent)}</p>
                      </div>
                      <div>
                        <p className="text-xs opacity-75 uppercase tracking-wider">Tax</p>
                        <p className="font-semibold">{formatPercent(order.taxPercent)}</p>
                      </div>
                      <div>
                ...
                {/* Customer */}
                <DetailCard title="Customer" className="bg-white border-gray-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-blue-50">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-700">{order.account.name}</p>
                      </div>
                    </div>
                    {order.contact && (
                      <>
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Contact Name</p>
                            <p className="text-sm font-medium text-gray-700">{order.contact.name}</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Contact Email</p>
                            <p className="text-sm font-medium text-gray-700">{order.contact.email ?? "—"}</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Contact Phone</p>
                            <p className="text-sm font-medium text-gray-700">{order.contact.phone ?? "—"}</p>
                          </div>
                        </div>
                      </>
                    )}
                        ...
                        <div className="space-y-3">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm font-medium text-gray-700">{order.billingName}</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-3">
                            <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-violet-50">
                            ...
                            <div>
                              <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Address</p>
                              <p className="text-sm font-medium text-gray-700">{order.billingStreet ?? "—"}</p>
                              {billingCityLine && <p className="text-sm text-gray-500">{billingCityLine}</p>}
                              {order.billingCountry && <p className="text-sm text-gray-500">{order.billingCountry}</p>}
                            </div>
                          </div>
                        </div>
                      </DetailCard>
                      ...
                      <DetailCard title="Shipping Address" className="bg-white border-gray-200">
                        <div className="space-y-3">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Name</p>
                              <p className="text-sm font-medium text-gray-700">{order.shippingName}</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-3">
                            <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-violet-50">
                            ...
                            <div>
                              <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Address</p>
                              <p className="text-sm font-medium text-gray-700">{order.shippingStreet ?? "—"}</p>
                              {shippingCityLine && <p className="text-sm text-gray-500">{shippingCityLine}</p>}
                              {order.shippingCountry && <p className="text-sm text-gray-500">{order.shippingCountry}</p>}
                            </div>
                          </div>
                        </div>
                      </DetailCard>
                      ...
                      {order.paymentTerms && (
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-gray-700">{order.paymentTerms}</p>
                          </div>
                        </div>
                      )}
                      {order.deliveryTerms && (
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-gray-700">{order.deliveryTerms}</p>
                          </div>
                        </div>
                      )}
                      {order.notes && (
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium text-gray-700 whitespace-pre-wrap">{order.notes}</p>
                          </div>
                        </div>
                      )}
                    </div>
                ...
                <DetailCard title="Owner" className="bg-white border-gray-200">
                  <div className="space-y-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Owner Name</p>
                        <p className="text-sm font-medium text-gray-700">{buildFullName(order.owner.firstName, order.owner.lastName)}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-sky-50">
                      ...
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Owner Email</p>
                        <p className="text-sm font-medium text-gray-700">{order.owner.email ?? "—"}</p>
                      </div>
                    </div>
                  </div>
                </DetailCard>
                ...
                <DetailCard title="System Information" className="bg-white border-gray-200">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-0.5">Last Modified</p>
                        <p className="text-sm font-medium text-gray-700">{formatDate(order.updatedAt)}</p>
                      </div>
                    </div>
                    {order.approvedBy && (
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-gray-700">{buildFullName(order.approvedBy.firstName, order.approvedBy.lastName)}</p>
                        </div>
                      </div>
                    )}
                  </div>
