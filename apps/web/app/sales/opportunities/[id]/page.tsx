...
import { useOpportunity, useUpdateOpportunityStage, useUpdateOpportunity, useDeleteOpportunity, useUpdateOpportunityLineItem, useDeleteOpportunityLineItem, useOpportunityLineItems, useOpportunityQuotes } from "@/hooks/useOpportunities"
...
function ProductLineItemsTable({
  ...
  const { data: lineItems = [], isLoading, isError } = useOpportunityLineItems(opportunityId)
  ...
  // Edit modal state
  const [editItem, setEditItem] = React.useState<OpportunityLineItem | null>(null)
  const [editOpen, setEditOpen] = React.useState(false)

  // Delete confirmation state
  const [deleteItem, setDeleteItem] = React.useState<OpportunityLineItem | null>(null)
  ...
  const [showChangePriceBookConfirm, setShowChangePriceBookConfirm] = React.useState(false)
  ...
  const handleSaveEdit = () => {
    ...
    updateLineItem.mutate(
      { opportunityId, lineItemId: editItem.id, data: { quantity: editQuantity, listPrice: editListPrice, discount: editDiscount, description: editDescription || null } },
      {
        onSuccess: () => {
    ...
    <div className="inline-flex items-center gap-2">
      ...
      <div className="inline-flex items-center rounded-full border border-gray-300 bg-white">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-black border-b-4 border-b-transparent rounded-full transition-all duration-200 hover:-translate-y-0.5 hover:border-b-gray-600 hover:bg-indigo-300 will-change-transform"
        >
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground py-6">Loading line items...</div>
  }

  if (isError) {
    return <div className="text-sm text-red-500 py-6">Failed to load line items.</div>
  }
  ...
  return (
    <>
      <DataTable<OpportunityLineItem>
        ...
        titleSuffix={
          priceBook ? (
            <Badge variant="outline" className="text-xs font-medium text-muted-foreground border-gray-400">
              {priceBook.name}
            </Badge>
          ) : undefined
        }
        customActions={(item) => (
          <div className="flex items-center gap-2">
            <Button size="icon" variant="outline" onClick={() => openEdit(item as OpportunityLineItem)}>
            ...
            {isAdmin && (
              <Button
                size="icon"
                variant="destructive"
                disabled={deleteLineItem.isPending && deleteItem?.id === (item as OpportunityLineItem).id}
                onClick={() => { setDeleteItem(item as OpportunityLineItem); setShowDeleteConfirm(true) }}
              >
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Edit Line Item — {editItem?.product?.name}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="number" min={0} max={100} step="0.01" value={editDiscount} onChange={(e) => setEditDiscount(Number(e.target.value))} />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="col-span-2 space-y-1.5">
              ...
              <Textarea value={editDescription} onChange={(e) => setEditDescription(e.target.value)} className="min-h-[80px]" />
            </div>
          </div>
          <DialogFooter>
      ...
      <ConfirmationDialog
        ...
        onOpenChange={(open) => { setShowChangePriceBookConfirm(open); if (!open) setSelectedPriceBookId("") }}
      ...
      <ConfirmationDialog
        ...
        onOpenChange={(open) => { setShowDeleteConfirm(open); if (!open) setDeleteItem(null) }}
      ...
      <Dialog open={priceBookDialogOpen} onOpenChange={(open) => { setPriceBookDialogOpen(open); if (!open) setSelectedPriceBookId("") }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{priceBook ? "Change Pricebook" : "Add Pricebook"}</DialogTitle>
          </DialogHeader>
          <div className="py-2 space-y-1.5">
            <Label>Select Pricebook</Label>
            <Select value={selectedPriceBookId} onValueChange={setSelectedPriceBookId}>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setPriceBookDialogOpen(false); setSelectedPriceBookId("") }}>Cancel</Button>
            <Button onClick={handlePriceBookSelectionNext} disabled={!selectedPriceBookId}>
              OK
            </Button>
          </DialogFooter>
        </DialogContent>
...
function OpportunityDetailContent() {
  ...
  const currentStageIndex = opportunity
    ...
    : 0
  const [selectedStageIndex, setSelectedStageIndex] = React.useState(currentStageIndex)
  React.useEffect(() => {
  ...
  const { data: quotesResponse } = useOpportunityQuotes(id, { page: 1, limit: 50 })
  ...
  const handleSaveFields = React.useCallback(() => {
    ...
    if (editNextStep !== apiNextStep)
      payload.nextStep = editNextStep || null
    ...
    if (editLeadSource !== apiLeadSource)
      payload.leadSource = editLeadSource === "__none__" ? null : editLeadSource
  ...
  }, [opportunity, editName, editCloseDate, editNextStep, editType, editLeadSource, updateOpportunity])
  ...
  if (isError || !opportunity) {
    return (
      <div className="p-6 space-y-4">
        ...
        <Button variant="outline" onClick={() => router.push("/sales/opportunities")}>
  ...
  const ownerName = buildFullName(opportunity.owner?.firstName, opportunity.owner?.lastName)
  const creatorName = buildFullName(opportunity.creator?.firstName, opportunity.creator?.lastName)
  const stageBadgeClass = STAGE_BADGE_CLASSES[opportunity.stage] || "bg-gray-100 text-gray-800 border-gray-200"
  ...
  const relatedQuotes: Quote[] = (quotesResponse?.data ?? []).map((q) => ({
    ...
    netAmount: typeof q.grandTotal === "number" ? q.grandTotal : Number(q.grandTotal) || 0,
    lineItemCount: 0,
          ...
          <TabsContent value="details">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                >
                  <InfoGrid columns={2}>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Badge className={stageBadgeClass}>{opportunity.stage}</Badge>
                    </div>
                    ...
                    {isEditing ? (
                      ...
                      <InfoField label="Lead Source" value={opportunity.leadSource || ""} />
                    )}
                    ...
                    {isEditing ? (
                      ...
                      <InfoField
                        ...
                        value={
                          opportunity.expectedCloseDate
                            ? new Date(opportunity.expectedCloseDate).toLocaleDateString("en-GB")
                            : ""
                        }
                      />
                    )}
                    ...
                    <InfoField
                      label="Next Step"
                      value={isEditing ? editNextStep : (opportunity.nextStep || "")}
                      editable={isEditing}
                    ...
                    <div className="col-span-2 space-y-1.5 pt-4 mt-4 border-t border-gray-200">
                      ...
                      <div className="text-sm sm:text-base leading-6 text-foreground font-medium whitespace-pre-wrap">
                        {opportunity.description || <span className="text-muted-foreground">N/A</span>}
                      </div>
                    </div>
                  </InfoGrid>
                </DetailCard>
              ...
              <div className="h-full">
                <DetailCard title="System Information" className="bg-gray-50/50 border-gray-200 h-full">
                  <InfoGrid columns={1}>
                    <InfoField
                      label="Opportunity Owner"
                      value={buildFullName(opportunity.owner.firstName, opportunity.owner.lastName)}
                    />
                    <InfoField
                      label="Created By"
                      value={buildFullName(opportunity.creator.firstName, opportunity.creator.lastName)}
                    />
                    <InfoField label="Created At" value={formatDateTime(opportunity.createdAt)} />
                    <InfoField label="Last Modified At" value={formatDateTime(opportunity.updatedAt)} />
                  </InfoGrid>
                </DetailCard>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="lg:col-span-3">
                <QuotesTable
                  ...
                  onQuoteClick={(quote) => router.push(`/sales/quotes/${quote.id}`)}
                />
              </div>
            </div>
          </TabsContent>
