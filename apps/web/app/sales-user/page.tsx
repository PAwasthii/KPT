      ...
      setSelectedLead(prev => {
        ...
        return {
          ...
          enquiries: prev.enquiries?.map(enq =>
            enq.id === enquiryId
              ? { ...enq, status: 'RESOLVED' as const, resolvedAt: new Date().toISOString() }
              : enq
          ) || []
        }
      })
              ...
              <div className="bg-white rounded-lg shadow-lg p-6 space-y-6">
                ...
                {selectedLead.enquiries && selectedLead.enquiries.length > 0 && (
                  <div className="border-t pt-4">
                    <Label className="text-sm font-semibold mb-3 block">Enquiries</Label>
                    <div className="space-y-3">
                      {selectedLead.enquiries
                        .sort((a, b) =>
                          new Date(b.enquiryCreatedAt).getTime() - new Date(a.enquiryCreatedAt).getTime()
                        )
                        .map((enquiry) => (
                          <div
                            key={enquiry.id}
                          ...
                          >
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <span className="text-xs text-gray-500">
                                    {new Date(enquiry.enquiryCreatedAt).toLocaleString()}
                                  </span>
                                  <Badge
                                    variant={
                                      enquiry.status === 'UNRESOLVED' ? 'default' :
                                      enquiry.status === 'RESOLVED' ? 'outline' : 'secondary'
                                    }
                                  >
                                    {enquiry.status}
                                  </Badge>
                                ...
                                {enquiry.landingPageCampaign && (
                                  <p className="text-sm font-medium text-gray-700">
                                    Campaign: {enquiry.landingPageCampaign.name}
                                  </p>
                                )}
                              </div>
                              {enquiry.status === 'UNRESOLVED' && (
                            ...
                            {enquiry.customFields && Object.keys(enquiry.customFields).length > 0 && (
                              <div className="mt-2 pt-2 border-t border-gray-200">
                                <p className="text-xs font-semibold text-gray-600 mb-1">Form Details:</p>
                      ...
                      {selectedLead.remarks.map((remark) => (
                        <div key={remark.id} className="bg-gray-50 p-3 rounded-lg">
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            {[remark.user.firstName, remark.user.lastName].filter(Boolean).join(' ') || 'Unknown'} - {new Date(remark.createdAt).toLocaleString()}
                          </p>
                        </div>
                      ))}
                    </div>
                ...
                <div className="grid gap-4">
                  {leads.map((lead) => {
                    const unresolvedCount =
                      lead.enquiries?.filter((e) => e.status === 'UNRESOLVED').length || 0;
                    return (
                      <div
                        key={lead.id}
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {unresolvedCount > 0 && (
                                <Badge variant="default" className="bg-indigo-500">
                                  {unresolvedCount} {unresolvedCount === 1 ? 'Enquiry' : 'Enquiries'}
                                </Badge>
                              )}
                            </div>
