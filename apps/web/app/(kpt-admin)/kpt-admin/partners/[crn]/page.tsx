...
interface DocItem { id: string; docType: string; fileName: string | null; verifyStatus: string; uploadedAt: string; verifiedAt: string | null; failReason: string | null; storagePath: string | null; }
interface FieldReport { assignedExecId: string | null; assignedExecName: string | null; shopDimensions: string | null; locationScore: string | null; roadVisibility: string | null; existingLines: string | null; marketPotential: string | null; nearestDealer: string | null; geoLat: number | null; geoLng: number | null; execNotes: string | null; scheduledDate: string | null; scheduleNote: string | null; visitedAt: string | null; verifiedAt: string | null; }
...
export default function PartnerDetailPage() {
  ...
  if (loading) return <div className="flex items-center justify-center h-64"><div className="w-8 h-8 border-2 border-[#2563EB] border-t-transparent rounded-full animate-spin" /></div>;
  if (!partner) return <div className="p-6"><p className="text-[14px] text-[#6B6B6B]">Partner not found. <Link href="/kpt-admin/partners" className="text-[#2563EB] hover:underline">Back to list</Link></p></div>;

  return (
    <div className="p-6">
      {/* Breadcrumb + header */}
      <div className="flex items-center gap-2 text-[13px] text-[#6B6B6B] mb-4">
        <Link href="/kpt-admin/partners" className="hover:text-[#2563EB]">Partners</Link>
        <span>›</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="bg-white border border-[#E8E8E6] rounded-[8px] p-5">
            ...
            <div className="mt-3 pt-3 border-t border-[#E8E8E6]">
              ...
              <div className="flex flex-wrap gap-1.5">
                {partner.productInterest.map(p => <span key={p} className="text-[11px] bg-[rgba(37,99,235,0.08)] text-[#2563EB] px-2 py-0.5 rounded">{p}</span>)}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {(partner.statusHistory ?? []).slice().reverse().map(h => (
                <div key={h.id} className="text-[12px]">
                  <p className="text-[#2D2D2D]"><span className="font-medium">{h.toStatus}</span></p>
                  {h.note && <p className="text-[#6B6B6B]">{h.note}</p>}
                  <p className="text-[#6B6B6B]">{fmt(h.createdAt)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* CENTER — Stage-specific detail */}
        <div className="space-y-4">
          {/* Field verification */}
          {partner.fieldReport && (
            <div className="bg-white border border-[#E8E8E6] rounded-[8px] p-5">
              <p className="text-[11px] uppercase tracking-[0.08em] text-[#6B6B6B] mb-3">Field Verification Report</p>
              ...
              <div className="grid grid-cols-2 gap-2 text-[12px] mb-3">
                <div><p className="text-[#6B6B6B]">Executive</p><p className="text-[#2D2D2D]">{partner.fieldReport.assignedExecName ?? '—'}</p></div>
                <div><p className="text-[#6B6B6B]">Visited</p><p className="text-[#2D2D2D]">{partner.fieldReport.visitedAt ? new Date(partner.fieldReport.visitedAt).toLocaleDateString('en-IN') : '—'}</p></div>
                <div><p className="text-[#6B6B6B]">Shop Dimensions</p><p className="text-[#2D2D2D]">{partner.fieldReport.shopDimensions ?? '—'}</p></div>
                ...
                <div><p className="text-[#6B6B6B]">Existing Lines</p><p className="text-[#2D2D2D]">{partner.fieldReport.existingLines ?? '—'}</p></div>
              </div>
              ...
              {partner.fieldReport.geoLat && partner.fieldReport.geoLng && (
                <a href={`https://www.google.com/maps?q=${partner.fieldReport.geoLat},${partner.fieldReport.geoLng}`} target="_blank" rel="noreferrer"
                  className="mt-2 block text-[12px] text-[#2563EB] hover:underline">View on Google Maps ({partner.fieldReport.geoLat.toFixed(4)}, {partner.fieldReport.geoLng.toFixed(4)})</a>
              )}
              <Link href={`/kpt-admin/field/${partner.crn}`} className="mt-3 block text-center border border-[#2563EB] text-[#2563EB] text-[12px] py-2 rounded-[4px] hover:bg-[rgba(37,99,235,0.08)] transition-colors">Assign / Reassign Field Exec</Link>
            </div>
          )}
                ...
                {partner.documents.map(doc => (
                  <div key={doc.id} className="flex items-center justify-between py-2 border-b border-[#E8E8E6] last:border-0">
                    <div>
                      ...
                      <p className="text-[11px] text-[#6B6B6B]">{doc.fileName ?? 'No file'} · {new Date(doc.uploadedAt).toLocaleDateString('en-IN')}</p>
        ...
        {/* RIGHT — Actions + approval log */}
        <div className="space-y-4">
          {/* Approval panel (Stage 4) */}
          {partner.currentStage === 4 && (
            <div className="bg-white border border-[#E8E8E6] rounded-[8px] p-5">
              <p className="text-[11px] uppercase tracking-[0.08em] text-[#2563EB] mb-3">Approval Decision</p>
              <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notes for applicant (required for reject / more info)…"
            ...
            <div className="space-y-3">
              {(partner.approvalLogs ?? []).slice().reverse().map(log => (
                <div key={log.id} className="text-[12px] border-l-2 border-[#E8E8E6] pl-3">
                  ...
                  <p className="text-[#6B6B6B]">{fmt(log.createdAt)}</p>
                </div>
              ))}
