...
export default function BookAOrderPage() {
  ...
  const [categories, setCategories] = useState<AakramanCategory[]>([]);
  ...
  const [cartQuantityInputs, setCartQuantityInputs] = useState<Record<number, string>>({});
  const [cartQuantityErrors, setCartQuantityErrors] = useState<Record<number, string>>({});
  const [showMobileMenu, setShowMobileMenu] = useState(false);
  const [stateSearchQuery, setStateSearchQuery] = useState("");

  // Firm details from localStorage
  const [firmDetails, setFirmDetails] = useState<AakramanOrderFirmDetails | null>(null);
  ...
  const addToCart = (product: AakramanProduct, quantity: number) => {
    ...
    if (!validationResult.success) {
      setQuantityErrors((prev) => ({
        ...prev,
        [product.id]: validationResult.error.issues[0]?.message || "Invalid quantity",
      }));
      return;
    }
  ...
  const updateCartQuantity = (productId: number, quantity: number) => {
    ...
    if (!validationResult.success) {
      setCartQuantityErrors((prev) => ({
        ...prev,
        [productId]: validationResult.error.issues[0]?.message || "Invalid quantity",
      }));
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <header className="sticky top-0 z-40 bg-indigo-400 shadow-md">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-2 sm:py-3">
          <div className="flex items-center justify-between gap- sm:gap-4">
            {/* Back Button and Logo */}
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <button
                onClick={handleBack}
                className="p-2 sm:p-2.5 rounded-lg bg-white/90 hover:bg-white transition-all duration-200 shadow-sm hover:shadow-md"
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button
                ...
                className="relative p-2 sm:p-2.5 rounded-lg bg-white/90 hover:bg-white transition-all duration-200 shadow-sm hover:shadow-md"
      ...
      <main className="max-w-7xl mx-auto px-3 py-2 mt-4">
        {filteredProducts.length === 0 ? (
          <div className="text-center py-12 sm:py-16 md:py-20">
            ...
            <p className="mt-4 text-gray-500 text-sm sm:text-base">No products found</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-5">
            {filteredProducts.map((product) => {
              ...
              return (
                <Card
                  key={product.id}
                  className="overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col bg-white h-full"
                >
                  ...
                  <CardContent className="p-2 flex flex-col flex-1">
                    ...
                    <div className="flex flex-wrap gap-1">
                      <Badge variant="secondary" className="text-xs mb-1 sm:text-[10px] md:text-xs bg-gray-100 text-gray-700 hover:bg-gray-100">
                        {product.code}
                      </Badge>
                    </div>
                    ...
                    <div className="mt-z space-y-2">
                      <div>
                        <Label htmlFor={`quantity-${product.id}`} className="text-xs sm:text-[11px] md:text-xs text-gray-700 font-medium">
                          Quantity
                        </Label>
                        <Input
                          ...
                          type="text"
                          inputMode="numeric"
                          value={quantityInputs[product.id] ?? (qty > 0 ? qty.toString() : "")}
                          onChange={(e) => {
                      ...
                      <Button
                        onClick={() => {
                          const inputValue = quantityInputs[product.id] ?? (qty > 0 ? qty.toString() : "");
                          const parsedQuantity = inputValue === "" ? 0 : Number.parseInt(inputValue, 10);
                        ...
                        size="sm"
                        className="w-full bg-indigo-400 hover:bg-indigo-500 text-black font-semibold h-8 sm:h-9 text-xs sm:text-sm"
                      >
                        <ShoppingCart className="w-3 h-3 sm:w-4 sm:h-4 mr-1" /> Add to Cart
                      </Button>
                    </div>
                  </CardContent>
                </Card>
      ...
      {/* Cart Slide-over */}
      <Dialog open={showCart} onOpenChange={(open) => !isSubmitting && setShowCart(open)}>
        <DialogContent className="w-[95vw] sm:max-w-md max-h-[90vh] overflow-y-auto">
          ...
          {isSubmitting && (
            <div className="absolute inset-0 bg-white/90 flex items-center justify-center rounded-lg">
              <div className="text-center">
                <Loader2 className="w-10 h-10 animate-spin text-yellow-500 mx-auto" />
                <p className="mt-3 text-sm font-medium text-gray-700">Placing your order...</p>
              </div>
            </div>
          )}
              ...
              cart.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1">
                      <Label htmlFor={`cart-quantity-${item.product.id}`} className="text-xs text-gray-700 font-medium">
                        Quantity
                      </Label>
                      <Input
                        ...
                        type="text"
                        inputMode="numeric"
                        value={cartQuantityInputs[item.product.id] ?? item.quantity.toString()}
                        onChange={(e) => {
                          const value = e.target.value;
                          setCartQuantityInputs((prev) => ({
                            ...prev,
