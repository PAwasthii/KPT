...
const customerDetailsSchema = z.object({
  ...
  contactNumber: z.string().regex(/^\d{10}$/, "Enter a valid 10 digit mobile number"),
  email: z.string().email("Enter a valid email address").optional().or(z.literal("")),
  city: z.string().min(1, "City is required"),
  state: z.string().min(1, "State is required"),
  pincode: z.string().regex(/^\d{6}$/, "Enter a valid 6 digit pincode").optional().or(z.literal("")),
  gst: z.string()
    .regex(/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/, "Enter a valid GST number (e.g., 29ABCDE1234F1Z5)")
    .optional()
    .or(z.literal("")),
});
...
export default function CustomerDetailsPage() {
  ...
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<keyof CustomerDetailsFormData, string>>>({});
  const [touchedFields, setTouchedFields] = useState<Partial<Record<keyof CustomerDetailsFormData, boolean>>>({});
  ...
  const handleInputChange = (field: keyof CustomerDetailsFormData, value: string) => {
  ...
  const handleSubmit = async () => {
    ...
    const allFields = Object.keys(formData) as (keyof CustomerDetailsFormData)[];
    setTouchedFields(
    ...
    try {
      ...
      if (err instanceof z.ZodError) {
        ...
        err.issues.forEach((issue) => {
          if (issue.path[0]) {
            errors[issue.path[0] as keyof CustomerDetailsFormData] = issue.message;
          }
        });
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-500 mt-2">
              Please fill in the customer details before proceeding to order booking
            </p>
          </CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                {touchedFields.firmName && fieldErrors.firmName && (
                  <p className="text-xs text-red-500 mt-1">{fieldErrors.firmName}</p>
                )}
              </div>
              ...
              <div>
                ...
                {touchedFields.ownerFirstName && fieldErrors.ownerFirstName && (
                  <p className="text-xs text-red-500 mt-1">{fieldErrors.ownerFirstName}</p>
                )}
              </div>
              ...
              <div>
                ...
                {touchedFields.ownerLastName && fieldErrors.ownerLastName && (
                  <p className="text-xs text-red-500 mt-1">{fieldErrors.ownerLastName}</p>
                )}
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("contactNumber", e.target.value.replace(/\D/g, "").slice(0, 10))
                  }
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    <div className="p-2 border-b sticky top-0 bg-popover z-10" onClick={(e) => e.stopPropagation()}>
            ...
            <Button
              ...
              disabled={isLoading}
              className="w-full bg-indigo-400 hover:bg-indigo-500 text-black font-semibold mt-6"
            >
              {isLoading ? (
                <>
