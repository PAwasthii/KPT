...
export default function WhatsappCampaignDetailPage() {
  ...
  const load = async () => {
    try {
      ...
      const stats = campaignData.deliveryStats || { total: 0, pending: 0, queued: 0, sent: 0, delivered: 0, read: 0, failed: 0 }
      ...
      const mappedCampaign = {
        ...
        startDate: status === "Draft" ? "-" : formatDate(campaignData.startDate),
        endDate: formatDate(campaignData.endDate),
        createdBy: campaignData.creator?.name || campaignData.creator?.email || "Unknown",
  ...
  if (error && !campaign) {
    return (
      <div className="container mx-auto p-6">
        ...
        <button onClick={handleBack} className="mt-4 underline">Back to campaigns</button>
      </div>
    )
  }
