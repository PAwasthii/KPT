...
import { CreateCampaignModal } from "@/components/whatsapp/create-campaign-modal"
...
function getStatusFromDeliveryStats(stats: { total: number; pending: number; queued: number; sent: number; delivered: number; read: number; failed: number }): string {
...
export default function WhatsappCampaignsPage() {
  ...
  const [editCampaignId, setEditCampaignId] = useState<number | undefined>(undefined)
  ...
  const loadCampaigns = async () => {
    try {
      ...
      const mapped: Campaign[] = data.map((c: any) => {
        const stats = c.deliveryStats || { total: 0, pending: 0, queued: 0, sent: 0, delivered: 0, read: 0, failed: 0 }
    ...
    <div className="container mx-auto p-6">
      {campaigns.length === 0 ? (
        <div className="rounded-md border p-8 text-center text-muted-foreground">
          No WhatsApp campaigns yet. <button onClick={() => setIsModalOpen(true)} className="underline">Create your first campaign</button>.
        </div>
      ) : null}
      <CampaignTable
        ...
        headerActions={
          ...
          <Button variant="outline" asChild>
            <Link href="/campaigns/whatsapp/opt-outs">
              Opt-Outs
            </Link>
          </Button>
          </div>
