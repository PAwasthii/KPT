...
import { SegmentFormModal, SegmentFormValues } from "@/components/segment-form-modal"
...
function transformSegmentToFormValues(segment: Segment): SegmentFormValues {
  return {
    ...
    rules: segment.rules.map((rule) => ({
      ...
      keywordIds:
        rule.ruleType === "KEYWORD"
          ? (Array.isArray(rule.value) ? rule.value.map((val) => Number(val)).filter((val) => !Number.isNaN(val)) : [])
          : [],
      values:
        rule.ruleType === "KEYWORD"
          ? ""
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Define reusable audiences for WhatsApp campaigns</p>
        </div>
            ...
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={5} className="text-center py-10 text-muted-foreground">
                  No segments yet. Create your first segment to start targeting campaigns.
                </TableCell>
              </TableRow>
            ) : (
              segments.map((segment) => (
                <TableRow key={segment.id}>
                  ...
                  <TableCell>
                    <p className="text-sm text-muted-foreground">{getRuleSummary(segment)}</p>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right space-x-2">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => handleEditClick(segment)} title="Edit">
