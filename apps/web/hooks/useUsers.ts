...
export const userKeys = {
  ...
  list: (filters: UserFilters & { page?: number; limit?: number }) => [...userKeys.lists(), { page: filters?.page, limit: filters?.limit, ...filters }] as const,
  details: () => [...userKeys.all, 'detail'] as const,
  detail: (id: number) => [...userKeys.details(), id] as const,
};

// Hooks for fetching users
export function useUsers(filters?: UserFilters & { page?: number; limit?: number }, options?: { enabled?: boolean }) {
  return useQuery({
...
export function useUsersWithPagination(filters?: UserFilters & { page?: number; limit?: number }, options?: { enabled?: boolean }) {
  const queryKey = userKeys.list(filters || {});
  
  const query = useQuery({
    queryKey,
    queryFn: () => {
      console.log('🔄 Fetching users - Page:', filters?.page, 'Limit:', filters?.limit);
