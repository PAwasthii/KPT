...
export const approvalKeys = {
  ...
  listAll: (params: { status?: string; targetObjectName?: string; page?: number; limit?: number }) =>
...
export function mapApiApprovalToApproval(item: ApprovalProcessApi): Approval {
  ...
  return {
    id: String(item.id),
    quoteNo: item.targetObjectName === 'QUOTE' ? `Quote ${item.targetRecordId}` : `—`,
    ...
    lastActorId: item.lastActorId != null ? String(item.lastActorId) : undefined,
    lastActorName,
  ...
  const query = useQuery({
    ...
    queryFn: async (): Promise<ApproverOption[]> => {
      const [adminRes, systemAdminRes] = await Promise.all([
        ...
        userService.getAllUsers({ role: 'SYSTEM_ADMIN', limit: 100 }),
      ])
      ...
      for (const u of [...systemAdminUsers, ...adminUsers]) {
        ...
        const name = [u.firstName, u.lastName].filter(Boolean).join(' ').trim() || u.email || `User ${u.id}`
