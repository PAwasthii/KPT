...
export const pricebookEntryKeys = {
  ...
  list: (filters: { productId: number }) => [...pricebookEntryKeys.lists(), filters] as const,
  listByPriceBookId: (filters: { priceBookId: number }) => [...pricebookEntryKeys.lists(), filters] as const,
...
export function usePricebookEntriesByPriceBookId(filters: { priceBookId: number }) {
  return useQuery({
    queryKey: pricebookEntryKeys.listByPriceBookId(filters),
    queryFn: () => pricebookEntryService.getPriceBookEntriesByPriceBookId(filters),
...
export function useCreatePricebookEntry() {
  ...
  return useMutation({
    mutationFn: (data: Partial<PriceBookEntry>) => pricebookEntryService.createPriceBookEntry(data),
    meta: { successMessage: 'Price book entry created successfully' },
    onSuccess: (newEntry) => {
      queryClient.invalidateQueries({ queryKey: pricebookEntryKeys.lists() });
      queryClient.setQueryData(pricebookEntryKeys.detail(newEntry.id), newEntry);
    },
  });
}
...
export function useUpdatePricebookEntry() {
  ...
  return useMutation({
    ...
    onSuccess: (updatedEntry) => {
      queryClient.invalidateQueries({ queryKey: pricebookEntryKeys.lists() });
      queryClient.setQueryData(pricebookEntryKeys.detail(updatedEntry.id), updatedEntry);
    },
  });
}
...
export function useDeletePricebookEntry() {
  ...
  return useMutation({
    ...
    onSuccess: (_, deletedId) => {
      queryClient.invalidateQueries({ queryKey: pricebookEntryKeys.lists() });
      queryClient.removeQueries({ queryKey: pricebookEntryKeys.detail(deletedId) });
    },
  });
}
