...
import { PriceBook, PaginatedApiResponse } from '../lib/api/types';

// Query keys
export const pricebookKeys = {
  all: ['pricebooks'] as const,
  lists: () => [...pricebookKeys.all, 'list'] as const,
  list: (filters: { page?: number; limit?: number }) => [...pricebookKeys.lists(), { page: filters?.page, limit: filters?.limit }] as const,
...
export function usePricebooksWithPagination(filters?: { page?: number; limit?: number }, options?: { enabled?: boolean }) {
...
export function useCreatePricebook() {
  ...
  return useMutation({
    mutationFn: (data: Partial<PriceBook>) => pricebookService.createPriceBook(data),
    ...
    onSuccess: (newPricebook) => {
      ...
      queryClient.setQueryData(pricebookKeys.detail(newPricebook.id), newPricebook);
    },
  });
}
