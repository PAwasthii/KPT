...
export const opportunityKeys = {
  ...
  lineItems: (opportunityId: number) => [...opportunityKeys.lineItemsAll(), opportunityId] as const,
...
export function useCreateOpportunity() {
  ...
  return useMutation({
    mutationFn: (data: CreateOpportunityInput) => opportunityService.create(data),
...
export function useDeleteOpportunityLineItem() {
  ...
  return useMutation({
    mutationFn: ({ opportunityId, lineItemId }: { opportunityId: number; lineItemId: number }) =>
    ...
    onSuccess: (_, { opportunityId }) => {
      queryClient.invalidateQueries({ queryKey: opportunityKeys.lineItems(opportunityId) })
...
export function useUpdateOpportunityLineItem() {
  ...
  return useMutation({
    ...
    }) => opportunityService.updateOpportunityLineItem(opportunityId, lineItemId, data),
    onSuccess: (_data, { opportunityId }) => {
      queryClient.invalidateQueries({ queryKey: opportunityKeys.detail(opportunityId) })
      queryClient.invalidateQueries({ queryKey: opportunityKeys.lineItems(opportunityId) })
    },
...
export function useOpportunityQuotes(
  ...
  return useQuery({
    ...
    queryFn: () => opportunityService.getOpportunityQuotes(opportunityId, { page, limit }),
    enabled: !!opportunityId,
    staleTime: 5 * 60 * 1000,
    refetchOnWindowFocus: false,
  })
    ...
    onSuccess: (result, _variables, _context) => {
      ...
      if (createdQuote) {
        ...
        queryClient.setQueryData<
          { data: OpportunityQuoteListItem[]; pagination: { totalItems: number; [key: string]: unknown } }
        >(queryKey, (old) => {
          ...
          return {
            ...
            pagination: prevPagination
              ...
              : { currentPage: 1, totalPages: 1, totalItems: 1, itemsPerPage: limit, hasNextPage: false, hasPreviousPage: false },
          }
        })
      }
